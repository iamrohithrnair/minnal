...
async function main() {
  ...
  section.name = 'minnal mobile concept';
  ...
  figma.closePlugin('Created minnal mobile concept screens.');
}
...
function buildSectionHeader(parent, x, y) {
  const eyebrow = createText({
    text: 'MINNAL · FIGMA MOBILE CONCEPT',
    x,
    y,
    width: 420,
    fontSize: 12,
  ...
  const title = createText({
    text: 'Current minnal iOS shell, translated into editable Figma screens',
    x,
    y: y + 20,
    width: 760,
    fontSize: 24,
...
function buildOnboarding(frame) {
  ...
  frame.appendChild(createText({
    text: 'minnal', x: 126, y: 232, width: 140,
  ...
  frame.appendChild(createText({
    text: 'Run minnal pair on your computer\nto generate a QR code.',
  ...
  addLabeledInput(frame, 'Pair Code', '6-digit code from minnal pair', 36, 680, 'key.fill');
...
function buildChat(frame) {
  ...
  frame.appendChild(createText({
    text: 'minnal', x: 40, y: 58, width: 160,
  ...
  addSystemBubble(frame, 20, 144, 288, 60, 'Connected to minnal over Tailscale.');
  ...
  frame.appendChild(createText({
    text: 'Message minnal…', x: 122, y: 790, width: 180,
