...
final class AppModel: ObservableObject {
    ...
    @Published var selectedServer: ServerCredential? {
        didSet {
            guard let selectedServer else {
                UserDefaults.standard.removeObject(forKey: "minnal.selected.host")
                UserDefaults.standard.removeObject(forKey: "minnal.selected.port")
                return
            }
            UserDefaults.standard.set(selectedServer.host, forKey: "minnal.selected.host")
            UserDefaults.standard.set(Int(selectedServer.port), forKey: "minnal.selected.port")
    ...
    private var client: MinnalClient?
    ...
    private let deviceId: String = {
        if let existing = UserDefaults.standard.string(forKey: "minnal.device.id") {
            return existing
        }
        let generated = "ios-" + UUID().uuidString.lowercased()
        UserDefaults.standard.set(generated, forKey: "minnal.device.id")
        return generated
    }()
    ...
    func loadSavedServers() async {
        ...
        let rememberedHost = UserDefaults.standard.string(forKey: "minnal.selected.host")
        let rememberedPort = UserDefaults.standard.integer(forKey: "minnal.selected.port")
    ...
    func pairAndSave() async {
        ...
        guard !code.isEmpty else {
            errorMessage = "Enter the 6-digit pairing code from minnal pair."
            return
        }
    ...
    func connectSelected() async {
        ...
        let newClient = MinnalClient(host: credential.host, port: credential.port, authToken: credential.authToken)
    ...
    private func applyConnectedServerInfo(_ info: ServerInfo) {
        ...
        serverName = info.serverName ?? "minnal"
...
@MainActor
private final class ClientDelegate: MinnalClientDelegate {
