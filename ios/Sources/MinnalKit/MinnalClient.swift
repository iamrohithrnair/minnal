...
@MainActor
public protocol MinnalClientDelegate: AnyObject {
...
@MainActor
public extension MinnalClientDelegate {
    func clientDidReplaceText(_ text: String) {}
    func clientDidReceiveToolInput(_ delta: String) {}
    func clientDidUpdateTokens(_ update: TokenUpdate) {}
    func clientDidChangeModel(model: String, provider: String?) {}
...
public actor MinnalClient {
    private let connection: MinnalConnection
    private nonisolated(unsafe) weak var _delegate: (any MinnalClientDelegate)?
    private var serverInfo = ServerInfo()
    private var eventTask: Task<Void, Never>?

    public init(host: String, port: UInt16 = 7643, authToken: String) {
        self.connection = MinnalConnection(host: host, port: port, authToken: authToken)
    }

    public func setDelegate(_ delegate: any MinnalClientDelegate) {
    ...
    private func handleEvent(_ event: MinnalConnection.Event) async {
        switch event {
    ...
    private nonisolated func callDelegate(_ block: @MainActor @Sendable (any MinnalClientDelegate) -> Void) async {
        await MainActor.run {
