...
The first implementation step for Minnal Desktop should be **Phase 0: a fullscreen blank white canvas**.
...
```text
packaging/linux/minnal-desktop.desktop
```

It expects a `minnal-desktop` binary to be available on `PATH`. For local testing after installing or copying the binary somewhere your desktop launcher can execute, copy the entry to your user applications directory:

```bash
mkdir -p ~/.local/share/applications
cp packaging/linux/minnal-desktop.desktop ~/.local/share/applications/
