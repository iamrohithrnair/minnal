...
Status: draft
Owner: minnal
Audience: minnal core, browser bridge authors, adapter authors

## Why this exists

minnal should expose a single first-class `browser` tool while remaining compatible with multiple browser automation backends:
...
The protocol in this document defines the **normalized contract** between minnal and a browser provider.

This is intentionally **not** a demand that every bridge speak exactly the same native command language. Instead:

- minnal defines a **core semantic layer** it can rely on
...
1. **One first-class tool in minnal**
...
3. **Capability negotiation**
   - minnal should know what each provider can and cannot do.
...
- **browser tool**: the user/model-facing minnal tool.
...
- **browser session**: the provider's isolated session or attachment scope for a minnal session.
...
- direct Rust trait calls inside minnal
...
minnal should not care whether a provider uses tabs, contexts, profiles, or remote targets internally.
It only needs a stable handle it can reuse.

### `session.ensure`

Creates or reuses a browser session for a minnal session.

Request:

```json
{
  "client_session_id": "minnal_session_123",
  "browser_preference": "auto",
  "isolation": "per_minnal_session",
  "attach": "prefer",
  "persist": true,
  "metadata": {
    "owner": "agent",
...
minnal must not assume identifier shape or encode browser semantics into them.

---

## Normalized core methods

These are the semantics minnal can rely on.
...
{
  "snapshot": {
    "format": "minnal.page_snapshot.v1",
    "root": {
      "node_id": "n1",
      "role": "document",
      "name": "Example Domain",
...
### Rule 3: minnal core should only rely on normalized methods by default
...
- a minnal-side adapter knows how to use them safely
...
Semantics or qualities that influence minnal behavior:
...
## Recommended minnal integration policy

The minnal `browser` tool should:
...
4. Should `page.snapshot` support multiple named formats beyond `minnal.page_snapshot.v1`?
