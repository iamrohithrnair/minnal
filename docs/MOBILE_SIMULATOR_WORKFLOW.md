...
This is the day-to-day workflow for humans and AI agents iterating on the minnal mobile application without a MacBook, Xcode, Apple iOS Simulator, or a physical iPhone.
...
```bash
cargo run -p minnal-mobile-sim -- find-node chat.send
```
...
```bash
cargo run -p minnal-mobile-sim -- assert-node chat.send --enabled true --role button
cargo run -p minnal-mobile-sim -- assert-node chat.draft --visible true --role composer
cargo run -p minnal-mobile-sim -- assert-node banner.status --label Status
```
...
```bash
cargo run -p minnal-mobile-sim -- assert-transition --type tap_node --contains chat.send
cargo run -p minnal-mobile-sim -- assert-effect --type send_message --contains "hello simulator"
```
...
cargo run -p minnal-mobile-sim -- tap pair.submit
cargo run -p minnal-mobile-sim -- assert-screen chat
cargo run -p minnal-mobile-sim -- assert-text "Connected to simulated minnal server."
cargo run -p minnal-mobile-sim -- set-field draft "hello simulator"
cargo run -p minnal-mobile-sim -- tap chat.send
cargo run -p minnal-mobile-sim -- assert-text "Simulated response to: hello simulator"
cargo run -p minnal-mobile-sim -- assert-transition --type tap_node --contains chat.send
...
```bash
cargo run -p minnal-mobile-sim -- status
cargo run -p minnal-mobile-sim -- find-node banner.error
cargo run -p minnal-mobile-sim -- log --limit 20
...
- Add a regression test in `minnal-mobile-sim` for each new automation method.
