...
This document audits the current provider, session, and shared-contract seams in the minnal workspace and recommends the next **realistic** crate moves that improve modularity without creating high-churn dependency cycles.
...
1. **Add a small `minnal-shared-contracts` crate** for the serde-only protocol/session overlap types that already act like shared contracts.
2. **After that, add a narrow `minnal-session-contracts` crate** for session metadata/replay/view structs that are widely reused but do not need the full `Session` runtime.
3. **If we want one more provider-side move before a larger provider refactor, extract the pure provider identity/selection layer** into `minnal-provider-core` or a small `minnal-provider-selection` crate.
...
- `minnal-provider-core`: no local workspace deps
- `minnal-provider-metadata`: no local workspace deps
- `minnal-provider-openrouter`: no local workspace deps
- `minnal-provider-gemini`: no local workspace deps
...
- `minnal-provider-metadata` holds stable login/profile catalog data
- `minnal-provider-core` holds route/cost/shared HTTP client/core value types
- `minnal-provider-openrouter` holds OpenRouter-specific catalog/cache/ranking/model-spec support
- `minnal-provider-gemini` holds Gemini Code Assist schema/types/support helpers
...
- either a new `crates/minnal-provider-selection`
- or a small `provider_identity` / `selection` module inside `minnal-provider-core`
...
### Option B: narrow `minnal-session-contracts`
...
### Option C: add `minnal-shared-contracts`
...
Create `crates/minnal-shared-contracts`.
...
Create `crates/minnal-session-contracts`.

Do this only after Phase 1, so session replay types can point at `minnal_shared_contracts::*` instead of `crate::protocol::*` or `crate::plan::*`.
...
1. Add `crates/minnal-shared-contracts` with serde-only types from `plan.rs` and the small protocol/session overlap set.
...
3. Re-measure touched-file compile times for:
   ...
   - `src/protocol.rs`
   - `src/provider/mod.rs`
4. If the new seam stays clean, follow with a narrow `minnal-session-contracts` extraction.
