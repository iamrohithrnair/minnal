...
This document describes a proposed evolution of minnal's UI architecture from the
...
Today, minnal is effectively organized like this:
...
- a independent minnal window attached to one session
...
- current independent minnal window
...
This can initially be provided through existing minnal control channels such as:
...
- Should workspace mode be a new command (`minnal workspace`) or a runtime mode of
  the normal client?
...
This gives minnal a portable built-in multi-session workspace without sacrificing
