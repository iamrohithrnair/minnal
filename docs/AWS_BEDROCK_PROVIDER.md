...
Minnal supports a native AWS Bedrock provider that talks directly to Bedrock Runtime with the AWS Rust SDK and `ConverseStream`.

## Configure credentials

Minnal supports two Bedrock auth styles:

- **Bedrock API key / bearer token**: easiest for local onboarding. Minnal stores the token in its config env file and sends it through the AWS SDK as `AWS_BEARER_TOKEN_BEDROCK`.
...
This saves `AWS_BEARER_TOKEN_BEDROCK` and `MINNAL_BEDROCK_REGION` to `~/.config/minnal/bedrock.env`.
...
export AWS_REGION=us-east-1
# Optional Minnal-specific overrides:
export MINNAL_BEDROCK_PROFILE=my-profile
export MINNAL_BEDROCK_REGION=us-east-1
```

If you rely on instance/container metadata credentials and have no local profile env vars, opt in explicitly:

```bash
export MINNAL_BEDROCK_ENABLE=1
export AWS_REGION=us-east-1
```
...
For AWS CLI console-login profiles, Minnal can also use credentials exported by:

```bash
aws configure export-credentials --profile my-profile --format env-no-export
```

Minnal does not store these exported session credentials; it asks the AWS CLI profile provider when the Bedrock provider initializes.
...
If you enable STS validation with `MINNAL_BEDROCK_VALIDATE_STS=1`, allow `sts:GetCallerIdentity`.

## Run Minnal with Bedrock
...
```bash
export MINNAL_BEDROCK_MAX_TOKENS=4096
export MINNAL_BEDROCK_TEMPERATURE=0.2
export MINNAL_BEDROCK_TOP_P=0.9
export MINNAL_BEDROCK_STOP_SEQUENCES='</done>,STOP'
```

## Model discovery

Minnal will use a static Bedrock model list immediately. When model prefetch/catalog refresh runs, it calls `ListFoundationModels` and `ListInferenceProfiles`, then caches results in Minnal's config directory. Cached Bedrock catalogs are region-scoped; if you switch `MINNAL_BEDROCK_REGION`/`AWS_REGION`, Minnal ignores the old-region cache and refreshes for the new region.
...
```bash
MINNAL_BEDROCK_LIVE_TEST=1 \
AWS_PROFILE=my-profile \
AWS_REGION=us-east-1 \
cargo test -p minnal --lib provider::bedrock::tests::bedrock_live_smoke_test -- --ignored
```
...
- Missing region: set `AWS_REGION` or `MINNAL_BEDROCK_REGION`.
