...
fn cleanup_background_task_files(task_id: &str) {
    let task_dir = std::env::temp_dir().join("minnal-bg-tasks");
...
fn ensure_test_minnal_home_if_unset() {
    ...
    let path = TEST_HOME.get_or_init(|| {
        let path = std::env::temp_dir().join(format!("minnal-test-home-{}", std::process::id()));
...
fn clear_persisted_test_ui_state() {
    if let Ok(home) = crate::storage::minnal_dir() {
...
fn create_minnal_repo_fixture() -> tempfile::TempDir {
