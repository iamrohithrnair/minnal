...
pub(super) fn semver() -> &'static str {
    ...
    SEMVER.get_or_init(|| format!("v{}", env!("MINNAL_SEMVER")))
}

/// True when this process is running from the stable release binary path.
/// Only matches the explicit ~/.minnal/builds/stable/minnal path, NOT
/// ~/.local/bin/minnal launcher path (which now points to current).
...
pub(super) fn binary_age() -> Option<String> {
    let git_date = env!("MINNAL_GIT_DATE");
