use anyhow::Result;
use clap::Parser;
use minnal::id::new_id;
use minnal::message::{Message, ToolDefinition};
use minnal::provider::{EventStream, Provider};
use minnal::tool::{Registry, ToolContext, ToolExecutionMode};
use serde_json::json;
use std::path::PathBuf;
use std::sync::Arc;

#[derive(Parser)]
#[command(name = "minnal-harness")]
...
fn create_temp_workspace() -> Result<PathBuf> {
    ...
    path.push(format!("minnal-harness-{}", new_id("run")));
    Ok(path)
}
