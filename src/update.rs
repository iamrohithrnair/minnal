...
mod tests {
    ...
    fn test_asset_name() {
        ...
        assert!(name.starts_with("minnal-"));
    }
    ...
    fn test_parse_sha256sums_accepts_standard_and_binary_lines() {
        ...
        assert_eq!(
            parsed.get("minnal-linux-x86_64.tar.gz").map(String::as_str),
            Some(digest_a.as_str())
        );
        assert_eq!(
            parsed.get("minnal-windows-aarch64.exe").map(String::as_str),
    ...
    fn test_verify_asset_checksum_text_accepts_matching_digest() {
        ...
        let contents = format!("{}  minnal-linux-x86_64.tar.gz\n", digest);
        verify_asset_checksum_text(&contents, "minnal-linux-x86_64.tar.gz", bytes).unwrap();
    }
    ...
    fn test_verify_asset_checksum_text_rejects_mismatch() {
        ...
        let contents = format!("{}  minnal-linux-x86_64.tar.gz\n", wrong);
        let err = verify_asset_checksum_text(&contents, "minnal-linux-x86_64.tar.gz", b"actual")
            .unwrap_err()
            .to_string();
    ...
    fn test_parse_sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-sha  minnal-linux-x86_64.tar.gz\n")
            .unwrap_err()
            .to_string();
