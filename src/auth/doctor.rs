...
pub fn recommended_actions(
    ...
    match assessment.state {
        AuthState::NotConfigured => actions.push(format!(
            "Connect it: `minnal login --provider {}`",
            provider.id
        )),
        AuthState::Expired
            if matches!(
        ...
        {
            actions.push(format!(
                "Re-run login; this provider cannot auto-refresh: `minnal login --provider {}`",
                provider.id
            ));
        }
        AuthState::Expired => actions.push(format!(
            "Refresh or replace the current login: `minnal login --provider {}`",
            provider.id
        )),
        AuthState::Available => {}
    }
        ...
        .and_then(|record| record.last_error.as_deref())
    {
        ...
        if lower.contains("invalid_grant") || lower.contains("refresh token") {
            actions.push(format!(
                "Replace the stale OAuth account/token: `minnal login --provider {}`",
                provider.id
            ));
    ...
    if assessment.state != AuthState::NotConfigured {
        match assessment.last_validation.as_ref() {
            None => actions.push(format!(
                "Run runtime verification: `minnal auth-test --provider {}`",
                provider.id
            )),
            Some(record) if !record.success => actions.push(format!(
                "Inspect runtime readiness: `minnal auth-test --provider {}`",
                provider.id
            )),
            Some(record) if validation_is_stale(record.checked_at_ms) => actions.push(format!(
                "Refresh stale runtime verification: `minnal auth-test --provider {}`",
                provider.id
            )),
            Some(_) => {}
        }
    ...
    if validation_result.is_some_and(|value| value != "validation passed") {
        actions.push(format!(
            "Re-run detailed auth diagnostics: `minnal auth-test --provider {}`",
            provider.id
        ));
    }
    ...
    {
        actions.push(format!(
            "For browser/callback issues, use the manual-safe flow: `minnal login --provider {} --print-auth-url`",
            provider.id
        ));
    }
