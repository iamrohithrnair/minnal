...
fn derive_session_provider_key_prefers_runtime_identity_over_transport() {
    let _lock = lock_env();
    let _runtime = EnvVarGuard::set("MINNAL_RUNTIME_PROVIDER", "azure-openai");
    let _namespace = EnvVarGuard::set("MINNAL_OPENROUTER_CACHE_NAMESPACE", "azure-cache");
    let _active = EnvVarGuard::set("MINNAL_ACTIVE_PROVIDER", "openrouter");
...
fn derive_session_provider_key_falls_back_to_openrouter_namespace() {
    let _lock = lock_env();
    let _runtime = EnvVarGuard::remove("MINNAL_RUNTIME_PROVIDER");
    let _namespace = EnvVarGuard::set("MINNAL_OPENROUTER_CACHE_NAMESPACE", "azure-openai");
    let _active = EnvVarGuard::set("MINNAL_ACTIVE_PROVIDER", "openrouter");
...
fn derive_session_provider_key_keeps_openai_compatible_profile_namespace() {
    let _lock = lock_env();
    let _runtime = EnvVarGuard::set("MINNAL_RUNTIME_PROVIDER", "openai-compatible");
    let _namespace = EnvVarGuard::set("MINNAL_OPENROUTER_CACHE_NAMESPACE", "zai");
    let _active = EnvVarGuard::set("MINNAL_ACTIVE_PROVIDER", "openrouter");
...
fn initial_session_context_uses_current_cwd_when_inserted() -> Result<()> {
    ...
    let first_dir = tempfile::Builder::new()
        .prefix("minnal-session-context-first-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let second_dir = tempfile::Builder::new()
        .prefix("minnal-session-context-second-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
...
fn initial_session_context_can_refresh_before_real_conversation() -> Result<()> {
    ...
    let original_cwd = std::env::current_dir().map_err(|e| anyhow!(e))?;
    let first_dir = tempfile::Builder::new()
        .prefix("minnal-session-context-stale-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let second_dir = tempfile::Builder::new()
        .prefix("minnal-session-context-real-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
...
fn initial_session_context_does_not_refresh_after_real_conversation() -> Result<()> {
    ...
    let original_cwd = std::env::current_dir().map_err(|e| anyhow!(e))?;
    let first_dir = tempfile::Builder::new()
        .prefix("minnal-session-context-original-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let second_dir = tempfile::Builder::new()
        .prefix("minnal-session-context-late-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
...
fn load_startup_stub_preserves_metadata_but_skips_heavy_vectors() -> Result<()> {
    ...
    let temp_home = tempfile::Builder::new()
        .prefix("minnal-startup-stub-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MINNAL_HOME", temp_home.path().as_os_str());
    ...
    session.record_env_snapshot(EnvSnapshot {
        ...
        minnal_version: "test".to_string(),
        minnal_git_hash: Some("abc123".to_string()),
        minnal_git_dirty: Some(false),
        os: "linux".to_string(),
...
fn load_for_remote_startup_preserves_messages_and_replay_but_skips_heavy_vectors() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("minnal-remote-startup-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MINNAL_HOME", temp_home.path().as_os_str());
...
fn test_create_marks_debug_when_test_session_env_enabled() {
    let _env_lock = lock_env();
    let _test_flag = EnvVarGuard::set("MINNAL_TEST_SESSION", "1");
...
fn test_create_not_debug_when_test_session_env_disabled() {
    let _env_lock = lock_env();
    let _test_flag = EnvVarGuard::set("MINNAL_TEST_SESSION", "0");
...
fn test_recover_crashed_sessions_preserves_debug_flag() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("minnal-recover-debug-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MINNAL_HOME", temp_home.path().as_os_str());
    let _test_flag = EnvVarGuard::set("MINNAL_TEST_SESSION", "0");
...
fn test_save_persists_full_session_content() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("minnal-session-save-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MINNAL_HOME", temp_home.path().as_os_str());
...
fn test_save_persists_compaction_state() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("minnal-session-compaction-save-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MINNAL_HOME", temp_home.path().as_os_str());
...
fn test_save_persists_provider_key() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("minnal-session-provider-key-save-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MINNAL_HOME", temp_home.path().as_os_str());
...
fn test_save_persists_reasoning_effort() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("minnal-session-reasoning-effort-save-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MINNAL_HOME", temp_home.path().as_os_str());
...
fn test_save_appends_journal_and_load_replays_it() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("minnal-session-journal-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MINNAL_HOME", temp_home.path().as_os_str());
...
fn test_save_checkpoints_after_full_mutation_and_clears_journal() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("minnal-session-checkpoint-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("MINNAL_HOME", temp_home.path().as_os_str());
