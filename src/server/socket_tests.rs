...
fn sibling_socket_path_roundtrip() {
    let main = std::path::PathBuf::from("/tmp/minnal.sock");
    let debug = std::path::PathBuf::from("/tmp/minnal-debug.sock");
...
fn cleanup_socket_pair_removes_main_and_debug_files() {
    ...
    let main = dir.join(format!("minnal-test-{}.sock", stamp));
    let debug = dir.join(format!("minnal-test-{}-debug.sock", stamp));
...
fn existing_server_start_errors_are_detected() {
    ...
    assert!(server_start_matches_existing_server(
        "Error: Refusing to replace active server socket at /run/user/1000/minnal.sock"
    ));
    assert!(!server_start_matches_existing_server(
