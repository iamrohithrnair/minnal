...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        if let Some(value) = &self.prev_home {
            crate::env::set_var("MINNAL_HOME", value);
        } else {
            crate::env::remove_var("MINNAL_HOME");
        }
        if let Some(value) = &self.prev_runtime_dir {
            crate::env::set_var("MINNAL_RUNTIME_DIR", value);
        } else {
            crate::env::remove_var("MINNAL_RUNTIME_DIR");
        }
        if let Some(value) = &self.prev_socket {
            crate::env::set_var("MINNAL_SOCKET", value);
        } else {
            crate::env::remove_var("MINNAL_SOCKET");
        }
    }
}
...
fn configure_test_env(root: &tempfile::TempDir) -> EnvGuard {
    ...
    let prev_socket = std::env::var_os("MINNAL_SOCKET");
    let home_dir = root.path().join("home");
    let runtime_dir = root.path().join("runtime");
    std::fs::create_dir_all(&home_dir).expect("create home dir");
    std::fs::create_dir_all(&runtime_dir).expect("create runtime dir");
    crate::env::set_var("MINNAL_HOME", &home_dir);
...
#[allow(
    ...
    reason = "test intentionally serializes process-wide MINNAL_HOME/env state across async recovery assertions"
)]
...
#[allow(
    clippy::await_holding_lock,
    reason = "test intentionally serializes process-wide MINNAL_HOME/env state across async startup assertions"
)]
async fn startup_ready_signal_is_not_blocked_by_headless_recovery_delay() -> Result<()> {
    ...
    let temp = tempfile::TempDir::new()?;
    let _env = configure_test_env(&temp);
    let _delay_guard = ScopedEnvVar::set("MINNAL_TEST_HEADLESS_STARTUP_RECOVERY_DELAY_MS", "500");
    ...
    let _ready_fd_guard = ScopedEnvVar::set("MINNAL_READY_FD", write_fd.to_string());
