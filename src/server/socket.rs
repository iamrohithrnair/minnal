...
pub fn socket_path() -> PathBuf {
    ...
    crate::storage::runtime_dir().join("minnal.sock")
}
...
pub(super) fn daemon_lock_path() -> PathBuf {
    crate::storage::runtime_dir().join("minnal-daemon.lock")
}
...
pub fn set_socket_path(path: &str) {
    crate::env::set_var("MINNAL_SOCKET", path);
}
...
/// `MINNAL_READY_FD`, and awaits a single byte on the read end. The server
...
pub async fn spawn_server_notify(cmd: &mut std::process::Command) -> Result<std::process::Child> {
    ...
    cmd.env("MINNAL_READY_FD", write_fd.to_string());
...
pub(super) fn format_server_start_error(
    ...
    if stderr_output.trim().is_empty() {
        format!(
            "Server exited before signalling ready ({}). Check logs at ~/.minnal/logs/",
            status
        )
    } else {
        format!(
...
/// Write a single byte to the fd in `MINNAL_READY_FD` and close it.
...
pub(super) fn signal_ready_fd() {
    #[cfg(unix)]
    {
        ...
        if let Ok(fd_str) = std::env::var("MINNAL_READY_FD") {
            crate::env::remove_var("MINNAL_READY_FD");
