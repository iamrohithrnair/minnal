...
fn create_hotkey_shortcut(use_alacritty: bool) -> Result<()> {
    ...
    let hotkey_dir = storage::minnal_dir()?.join("hotkey");
    ...
    let _ = std::process::Command::new("powershell")
        .args([
            "-NoProfile",
            "-Command",
            "Get-Process powershell, pwsh -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -like '*minnal-hotkey*' } | Stop-Process -Force -ErrorAction SilentlyContinue",
        ])
        .output();

    let ps1_path = hotkey_dir.join("minnal-hotkey.ps1");
    let ps1_content = format!(
        r#"# minnal Alt+; global hotkey listener
# Auto-generated by minnal setup-hotkey. Runs at login via startup shortcut.
...
"#,
    ...
    let vbs_path = hotkey_dir.join("minnal-hotkey-launcher.vbs");
...
$shortcut = $shell.CreateShortcut("{startup_dir}\minnal-hotkey.lnk")
...
fn nudge_hotkey(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Set up Alt+; to launch minnal from anywhere?\x1b[0m              \x1b[36m│\x1b[0m"
    );
    eprintln!(
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m    Opens minnal in {:<39}    \x1b[36m│\x1b[0m",
        ...
        "y" | "yes" => {
            ...
            match create_hotkey_shortcut(using_alacritty) {
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "    You can set it up manually later with: \x1b[1mminnal setup-hotkey\x1b[0m"
                    );
                    eprintln!();
                    false
                }
...
fn nudge_alacritty(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!("\x1b[36m┌─────────────────────────────────────────────────────────────┐\x1b[0m");
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Alacritty: the fastest terminal for minnal\x1b[0m               \x1b[36m│\x1b[0m"
    );
    eprintln!(
        "\x1b[36m│\x1b[0m                                                             \x1b[36m│\x1b[0m"
    );
                ...
                Ok(()) => {
                    ...
                    if state.hotkey_configured {
                        ...
                        match create_hotkey_shortcut(true) {
                            Ok(()) => {
                                eprintln!(
                                    "  \x1b[32m✓\x1b[0m Hotkey updated: \x1b[1mAlt+;\x1b[0m → Alacritty + minnal"
                                );
                            }
                            Err(e) => {
...
fn prompt_try_it_out(installed_alacritty: bool) {
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    Press \x1b[1mAlt+;\x1b[0m from anywhere to launch minnal.                \x1b[32m│\x1b[0m"
    );
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    \x1b[90m(Starting minnal normally in 3 seconds...)\x1b[0m                 \x1b[32m│\x1b[0m"
    );
    eprintln!("\x1b[32m└─────────────────────────────────────────────────────────────┘\x1b[0m");
    eprintln!();
...
pub(super) fn run_setup_hotkey_windows() -> Result<()> {
    ...
    eprintln!(
        "  Setting up \x1b[1mAlt+;\x1b[0m → {} + minnal...",
        terminal_name
    );
...
pub(super) fn create_windows_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    ...
    let shortcut_path = format!("{}\\Desktop\\minnal.lnk", desktop_dir);
...
$shortcut.Description = "minnal - AI coding agent"
$shortcut.Save()
Write-Output "OK"
"#,
