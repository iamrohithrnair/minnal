        ...
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            other => {
                crate::logging::warn(&format!(
                    "Unknown MINNAL_OPENAI_TRANSPORT '{}'; using auto. Use: auto, websocket, or https.",
                    other
                ));
                Self::Auto
            }
...
impl OpenAIProvider {
    ...
    pub fn new(credentials: CodexCredentials) -> Self {
        ...
        let mut model =
            std::env::var("MINNAL_OPENAI_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string());
        ...
        let prompt_cache_key = std::env::var("MINNAL_OPENAI_PROMPT_CACHE_KEY")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = std::env::var("MINNAL_OPENAI_PROMPT_CACHE_RETENTION")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = match prompt_cache_retention.as_deref() {
            ...
            Some(other) => {
                crate::logging::info(&format!(
                    "Warning: Unsupported MINNAL_OPENAI_PROMPT_CACHE_RETENTION '{}'; expected 'in_memory' or '24h'",
                    other
                ));
                None
            }
    ...
    fn parse_max_output_tokens(raw: Option<&str>) -> Option<u32> {
        ...
        match raw.parse::<u32>() {
            ...
            Err(_) => {
                crate::logging::warn(&format!(
                    "Invalid MINNAL_OPENAI_MAX_OUTPUT_TOKENS='{}'; using default {}",
    ...
    fn load_max_output_tokens() -> Option<u32> {
        let raw = std::env::var("MINNAL_OPENAI_MAX_OUTPUT_TOKENS").ok();
        ...
        if raw.is_some() {
            match parsed {
                ...
                None => crate::logging::info(
                    "OpenAI max_output_tokens disabled (MINNAL_OPENAI_MAX_OUTPUT_TOKENS=0)",
                ),
            }
        }
        parsed
