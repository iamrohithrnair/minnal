...
impl MinnalProvider {
    ...
    fn apply_runtime_profile() {
        let _ = crate::provider::activation::ProviderActivation::minnal_subscription(
...
impl Default for MinnalProvider {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Provider for MinnalProvider {
    async fn complete(
        &self,
        messages: &[Message],
    ...
    fn uses_minnal_compaction(&self) -> bool {
        self.inner.uses_minnal_compaction()
    }
...
mod tests {
    ...
    #[test]
    fn minnal_provider_enables_subscription_runtime_mode() {
        let _guard = crate::storage::lock_test_env();
        crate::subscription_catalog::clear_runtime_env();
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");

        runtime.block_on(async {
            let provider = MinnalProvider::new();
    ...
    #[test]
    fn minnal_provider_name_and_default_model_are_curated() {
        let _guard = crate::storage::lock_test_env();
        crate::subscription_catalog::clear_runtime_env();
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");
