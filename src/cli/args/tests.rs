...
fn model_list_subcommand_parses() {
    let args = Args::try_parse_from(["minnal", "model", "list", "--json", "--verbose"]).unwrap();
    match args.command {
...
fn session_rename_subcommand_parses() {
    ...
    let args = Args::try_parse_from(["minnal", "session", "rename", "fox", "--clear"]).unwrap();
    match args.command {
        Some(Command::Session(SessionCommand::Rename {
            session,
            name,
...
fn login_no_browser_flag_parses() {
    let args = Args::try_parse_from(["minnal", "login", "--no-browser"]).unwrap();
    match args.command {
        Some(Command::Login {
            account,
            no_browser,
    ...
    let args = Args::try_parse_from(["minnal", "login", "--headless"]).unwrap();
    match args.command {
...
fn login_scriptable_flags_parse() {
    let args = Args::try_parse_from(["minnal", "login", "--print-auth-url", "--json"]).unwrap();
    match args.command {
        Some(Command::Login {
            print_auth_url,
            json,
    ...
    let args = Args::try_parse_from(["minnal", "login", "--auth-code", "abc123"]).unwrap();
    match args.command {
...
fn quiet_global_flag_parses() {
    let args = Args::try_parse_from(["minnal", "--quiet", "model", "list"]).unwrap();
    assert!(args.quiet);
}

#[test]
fn run_json_subcommand_parses() {
    let args = Args::try_parse_from(["minnal", "run", "--json", "hello"]).unwrap();
    match args.command {
        Some(Command::Run {
            json,
            ndjson,
...
fn run_ndjson_subcommand_parses() {
    let args = Args::try_parse_from(["minnal", "run", "--ndjson", "hello"]).unwrap();
    match args.command {
        Some(Command::Run {
            json,
            ndjson,
...
fn version_subcommand_parses() {
    let args = Args::try_parse_from(["minnal", "version", "--json"]).unwrap();
    match args.command {
...
fn usage_subcommand_parses() {
    let args = Args::try_parse_from(["minnal", "usage", "--json"]).unwrap();
    match args.command {
...
fn auth_status_subcommand_parses() {
    let args = Args::try_parse_from(["minnal", "auth", "status", "--json"]).unwrap();
    match args.command {
...
fn auth_doctor_subcommand_parses() {
    let args = Args::try_parse_from(["minnal", "auth", "doctor", "openai", "--validate", "--json"])
        .unwrap();
    match args.command {
...
fn provider_list_subcommand_parses() {
    let args = Args::try_parse_from(["minnal", "provider", "list", "--json"]).unwrap();
    match args.command {
...
fn provider_current_subcommand_parses() {
    let args = Args::try_parse_from(["minnal", "provider", "current", "--json"]).unwrap();
    match args.command {
...
fn restart_save_subcommand_parses() {
    let args = Args::try_parse_from(["minnal", "restart", "save"]).unwrap();
    match args.command {
...
fn restart_save_auto_restore_flag_parses() {
    let args = Args::try_parse_from(["minnal", "restart", "save", "--auto-restore"]).unwrap();
    match args.command {
        Some(Command::Restart {
