        ...
        std::thread::spawn(move || {
                ...
                && update_available
            {
                if auto_update {
                    ...
                    logging::info("Update available! Run `minnal update` or `/reload` to update.");
                }
            }
            logging::info(&format!(
...
fn report_main_error(error: &anyhow::Error) {
    ...
    if let Some(session_id) = terminal::get_current_session() {
        ...
        output::stderr_info(format!("  minnal --resume {}", session_id));
        output::stderr_blank_line();
    }
}
...
mod tests {
    ...
    fn auto_install_allowed_without_live_terminal() {
        let args = parse_args(&["minnal", "login"]);
    ...
    fn auto_install_respects_explicit_disable_even_without_terminal() {
        let mut args = parse_args(&["minnal", "login"]);
    ...
    fn update_command_still_skips_background_check_before_auto_install_logic() {
        let args = parse_args(&["minnal", "update"]);
