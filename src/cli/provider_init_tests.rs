...
fn test_provider_choice_arg_values() {
    assert_eq!(ProviderChoice::Minnal.as_arg_value(), "minnal");
...
#[test]
fn test_init_provider_minnal_delegates_runtime_profile_to_wrapper() {
    let _guard = lock_env();
    let _env_guard = crate::storage::lock_test_env();
    crate::subscription_catalog::clear_runtime_env();
    crate::env::remove_var("MINNAL_OPENROUTER_MODEL");
    ...
    let provider = runtime
        .block_on(init_provider(&ProviderChoice::Minnal, None))
        .expect("init minnal provider");

    assert_eq!(provider.name(), "Minnal Subscription");
    ...
    assert_eq!(
        std::env::var("MINNAL_ACTIVE_PROVIDER").ok().as_deref(),
        Some("openrouter")
    );
    assert_eq!(
        std::env::var("MINNAL_RUNTIME_PROVIDER").ok().as_deref(),
        Some("minnal")
    );
    assert_eq!(
        std::env::var("MINNAL_FORCE_PROVIDER").ok().as_deref(),
        Some("1")
    );
...
fn choice_for_login_provider_round_trips_core_targets() {
    assert_eq!(
        choice_for_login_provider(provider_catalog::MINNAL_LOGIN_PROVIDER),
        Some(ProviderChoice::Minnal)
    );
    assert_eq!(
...
fn resolved_profile_default_model_uses_openai_compatible_override() {
    ...
    crate::env::set_var("MINNAL_OPENAI_COMPAT_DEFAULT_MODEL", "llama3.2");
