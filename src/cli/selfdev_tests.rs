...
fn set_socket_test_env(socket_path: &Path, runtime_dir: &Path) -> EnvVarGuard {
    let guard = EnvVarGuard::capture(&["MINNAL_SOCKET", "MINNAL_RUNTIME_DIR"]);
    crate::server::set_socket_path(socket_path.to_str().expect("utf8 socket path"));
    crate::env::set_var("MINNAL_RUNTIME_DIR", runtime_dir);
    guard
}
...
impl TestEnvGuard {
    fn new() -> anyhow::Result<Self> {
        ...
        let temp_home = tempfile::Builder::new()
            .prefix("minnal-selfdev-test-home-")
            .tempdir()?;
        let env = EnvVarGuard::capture(&["MINNAL_HOME", "MINNAL_TEST_SESSION"]);

        crate::env::set_var("MINNAL_HOME", temp_home.path());
        crate::env::set_var("MINNAL_TEST_SESSION", "1");
...
fn isolated_launcher_env() -> (
    ...
    let lock = lock_env();
    let temp = tempfile::tempdir().expect("tempdir");
    let env = EnvVarGuard::capture(&["MINNAL_INSTALL_DIR", "MINNAL_HOME", "HOME", "USERPROFILE"]);
    crate::env::set_var("HOME", temp.path());
    crate::env::set_var("USERPROFILE", temp.path());
    crate::env::remove_var("MINNAL_INSTALL_DIR");
...
#[test]
fn test_launcher_dir_uses_trimmed_install_dir_before_minnal_home() {
    let (_lock, _env, temp) = isolated_launcher_env();
    let install_dir = temp.path().join("install bin");
    let minnal_home = temp.path().join("minnal-home");
    set_var(
        "MINNAL_INSTALL_DIR",
        format!("  {}  ", install_dir.display()),
    );
    set_var("MINNAL_HOME", &minnal_home);
...
fn test_selfdev_build_command_prefers_repo_wrapper_when_present() {
    ...
    assert!(command.contains("dev_cargo.sh' build --profile selfdev -p minnal --bin minnal"));
    assert!(!command.contains("minnal-desktop"));
    assert!(build.display.contains("-p minnal --bin minnal"));
    assert!(!build.display.contains("minnal-desktop"));
}
...
fn test_selfdev_build_command_falls_back_to_cargo_when_wrapper_missing() {
    ...
    assert_eq!(build.args.first().map(String::as_str), Some("-lc"));
    let command = build.args.get(1).expect("shell command");
    assert!(command.contains("cargo build --profile selfdev -p minnal --bin minnal"));
    assert!(!command.contains("minnal-desktop"));
}
...
fn test_selfdev_build_command_can_target_all() {
    ...
    assert!(
        build
            .display
            .contains("-p minnal-desktop --bin minnal-desktop")
    );
}
