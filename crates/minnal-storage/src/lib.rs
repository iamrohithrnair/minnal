...
///
/// Can be overridden with `$MINNAL_RUNTIME_DIR`.
...
fn fallback_runtime_dir() -> PathBuf {
    std::env::temp_dir().join(format!("minnal-{}", runtime_user_discriminator()))
}
...
fn ensure_private_runtime_dir(path: &Path) {
    ...
    #[cfg(unix)]
    {
        let _ = minnal_core::fs::set_directory_permissions_owner_only(path);
    }
}

pub fn minnal_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("MINNAL_HOME") {
...
pub fn logs_dir() -> Result<PathBuf> {
    Ok(minnal_dir()?.join("logs"))
}

/// Resolve minnal's app-owned config directory.
///
/// Default location is the platform config dir + `minnal` (for example
/// `~/.config/minnal` on Linux). When `MINNAL_HOME` is set, sandbox this under
/// `$MINNAL_HOME/config/minnal` so self-dev/tests do not leak into the user's
/// real config directory.
pub fn app_config_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("MINNAL_HOME") {
        return Ok(PathBuf::from(path).join("config").join("minnal"));
    }

    let config_dir =
        dirs::config_dir().ok_or_else(|| anyhow::anyhow!("No config directory found"))?;
    Ok(config_dir.join("minnal"))
}

/// Resolve a path under the user's home directory, but sandbox it under
/// `$MINNAL_HOME/external/` when `MINNAL_HOME` is set.
///
...
pub fn harden_user_config_permissions() {
    if let Some(config_dir) = dirs::config_dir() {
        let minnal_config_dir = config_dir.join("minnal");
        if minnal_config_dir.exists() {
            let _ = minnal_core::fs::set_directory_permissions_owner_only(&minnal_config_dir);
        }
    }

    if let Ok(minnal_home) = minnal_dir()
        && minnal_home.exists()
    {
        let _ = minnal_core::fs::set_directory_permissions_owner_only(&minnal_home);
    }
}
...
pub fn harden_secret_file_permissions(path: &Path) {
    if let Some(parent) = path.parent() {
        let _ = minnal_core::fs::set_directory_permissions_owner_only(parent);
    }
    if path.exists() {
        let _ = minnal_core::fs::set_permissions_owner_only(path);
    }
}

/// Validate an external auth file managed by another tool before reading it.
///
/// minnal intentionally avoids mutating these files. We also reject obvious risky
...
pub fn ensure_dir(path: &Path) -> Result<()> {
    if !path.exists() {
        ...
        minnal_core::fs::set_directory_permissions_owner_only(path)?;
    }
    Ok(())
}
...
pub fn write_text_secret(path: &Path, content: &str) -> Result<()> {
    ...
    if let Some(parent) = path.parent() {
        minnal_core::fs::set_directory_permissions_owner_only(parent)?;
    }
