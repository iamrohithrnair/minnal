...
const DISABLE_ENV: &str = "MINNAL_DISABLE_POWER_INHIBIT";

/// Best-effort inhibitor that keeps laptops awake while Minnal is actively
...
impl PowerInhibitor {
    ...
    fn acquire(&mut self) {
        ...
        match build_inhibit_command(platform).spawn() {
            ...
            Err(error) => {
                eprintln!("minnal: failed to acquire power inhibitor: {error}");
                self.available = false;
            }
        }
    }
...
fn build_linux_systemd_inhibit_command() -> Command {
    ...
    command
        ...
        .arg("--who=minnal")
        .arg("--why=Minnal is streaming or processing active work")
        .arg("sleep")
        .arg("infinity")
        .stdin(Stdio::null())
        .stdout(Stdio::null())
...
mod tests {
    ...
    fn linux_inhibitor_blocks_sleep_and_lid_switch() {
        ...
        assert!(args.contains(&"--who=minnal".to_string()));
