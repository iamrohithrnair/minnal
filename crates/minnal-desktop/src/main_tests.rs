...
fn single_session_header_exposes_desktop_binary_and_version() {
    ...
    let build_version = option_env!("MINNAL_DESKTOP_VERSION").unwrap_or(env!("CARGO_PKG_VERSION"));

    assert!(key.version.contains(build_version));
    assert!(
        key.version.contains("minnal-desktop") || key.version.contains("minnal_desktop"),
...
fn single_session_visual_state_smoke_covers_markdown_spinner_and_switcher() {
    ...
    assert_visual_text_contains(
        &switcher_key,
        "loading recent sessions from ~/.minnal/sessions...",
    );
}
...
fn single_session_tool_events_expand_context_and_collapse_previous_call() {
    ...
    app.apply_session_event(session_launch::DesktopSessionEvent::ToolInput {
        delta: r#"{"command":"cargo test -p minnal-desktop","timeout":120000,"intent":"Run desktop tests"}"#.to_string(),
    });
    ...
    assert!(body.contains("$ cargo test -p minnal-desktop"));
...
fn desktop_resume_args_are_parsed() {
    assert_eq!(
        desktop_resume_session_id_from_args(["minnal-desktop", "--resume", "session_beta"]),
        Some("session_beta".to_string())
    );
    assert_eq!(
        desktop_resume_session_id_from_args(["minnal-desktop", "--resume=session_gamma"]),
        Some("session_gamma".to_string())
    );
    assert_eq!(
        desktop_resume_session_id_from_args(["minnal-desktop"]),
        None
    );
}
    ...
    match action {
        ...
        QueueTraceAction::Reloading => {
            app.apply_session_event(session_launch::DesktopSessionEvent::Reloading {
                new_socket: Some("/tmp/minnal-reload-model-test.sock".to_string()),
            });
            None
        }
        QueueTraceAction::Reloaded => {
    ...
    let drained = apply_session_event_batch_then_auto_drain(
        ...
        vec![
            session_launch::DesktopSessionEvent::Reloading {
                new_socket: Some("/tmp/minnal-reload-event-loop-test.sock".to_string()),
            },
...
fn single_session_event_loop_multiple_reloads_preserve_queued_prompt_order() {
    ...
    assert_eq!(
        apply_session_event_batch_then_auto_drain(
            ...
            vec![
                session_launch::DesktopSessionEvent::Reloading {
                    new_socket: Some("/tmp/minnal-reload-one.sock".to_string()),
                },
                session_launch::DesktopSessionEvent::Reloaded {
                    session_id: "reload-model-session".to_string(),
                },
    ...
    assert_eq!(
        apply_session_event_batch_then_auto_drain(
            ...
            vec![
                session_launch::DesktopSessionEvent::Reloading {
                    new_socket: Some("/tmp/minnal-reload-two.sock".to_string()),
                },
                session_launch::DesktopSessionEvent::Reloaded {
                    session_id: "reload-model-session".to_string(),
                },
...
fn headless_chat_smoke_message_parses_hidden_flag() {
    ...
    assert_eq!(
        headless_chat_smoke_message(&["minnal-desktop".to_string()]),
        None
    );
}
...
fn desktop_startup_flags_enable_logging_and_benchmark_mode() {
    let args = vec!["minnal-desktop".to_string(), "--startup-log".to_string()];
    ...
    assert!(startup_benchmark_requested(&args));
    assert!(!startup_log_requested(&["minnal-desktop".to_string()]));
...
fn single_session_reload_event_keeps_worker_state_processing() {
    let mut app = SingleSessionApp::new(None);
    app.apply_session_event(session_launch::DesktopSessionEvent::Reloading {
        new_socket: Some("/tmp/minnal-reload.sock".to_string()),
    });
...
fn desktop_mode_defaults_to_single_session_and_gates_workspace_prototype() {
    assert_eq!(
        desktop_mode_from_args(["minnal-desktop"]),
        DesktopMode::SingleSession
    );
    assert_eq!(
        desktop_mode_from_args(["minnal-desktop", "--workspace"]),
