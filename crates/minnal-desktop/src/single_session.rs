...
impl SingleSessionApp {
    ...
    pub(crate) fn status_title(&self) -> String {
        ...
        format!(
            "Minnal Desktop · single session · {title} · Enter send · Shift+Enter newline · Ctrl+Enter queue · Ctrl+P sessions · Ctrl+Shift+M models · Ctrl+Shift+S info · Ctrl+; spawn · Esc interrupt · --workspace for Niri layout"
        )
    }
...
pub(crate) fn single_session_surface(
    ...
    workspace::Surface {
        ...
        title: session
            ...
            .unwrap_or_else(|| "new minnal session".to_string()),
...
mod tests {
    ...
    fn bash_tool_rendering_shows_intent_command_and_background_flag() {
        let lines = rendered_tool_text(
            "▾ bash running\n  input: {\"intent\":\"run the desktop tests\",\"command\":\"cargo test -p minnal-desktop\",\"run_in_background\":true}",
            true,
        );
        assert_eq!(
            lines,
            vec![
                "  ● bash · running · intent: run the desktop tests · $ cargo test -p minnal-desktop · background: yes",
                "  ╭────────────────────────────────────────────────────────────────────╮",
    ...
    fn tool_result_content_renders_inside_inline_widget() {
        let lines = rendered_tool_text(
            "▾ bash failed: tests failed\n  input: {\"command\":\"cargo test -p minnal-desktop\"}\n  error[E0425]: cannot find value `foo` in this scope\n  test result: FAILED",
            true,
        );

        assert_eq!(
            lines[0],
            "  ✕ bash · failed · tests failed · $ cargo test -p minnal-desktop"
        );
        assert_eq!(
            lines[1],
            "  ╭────────────────────────────────────────────────────────────────────╮"
    ...
    fn inactive_tool_result_compacts_to_metadata_only() {
        ...
        assert_eq!(
            lines,
            vec!["  ✓ bash · done · tests passed · $ cargo test -p minnal-desktop"]
        );
    }
