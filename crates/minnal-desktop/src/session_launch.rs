...
pub fn launch_resume_session(session_id: &str, title: &str) -> Result<()> {
    let title = format!("minnal · {}", compact_title(title));
...
pub fn launch_new_session() -> Result<()> {
    let candidates = terminal_candidates("minnal · new session", &["--fresh-spawn"]);
...
pub fn spawn_fresh_server_session(
    ...
    std::thread::Builder::new()
        .name("minnal-desktop-fresh-session".to_string())
        .spawn(move || {
            if let Err(error) =
...
pub fn spawn_message_to_session(
    ...
    let handle = DesktopSessionHandle { command_tx };
    std::thread::Builder::new()
        .name("minnal-desktop-session-message".to_string())
        .spawn(move || {
...
pub fn spawn_cycle_model(
    ...
    std::thread::Builder::new()
        .name("minnal-desktop-cycle-model".to_string())
        .spawn(move || {
...
pub fn spawn_cycle_reasoning_effort(
    ...
    std::thread::Builder::new()
        .name("minnal-desktop-cycle-effort".to_string())
        .spawn(move || {
...
pub fn spawn_load_model_catalog(
    ...
    std::thread::Builder::new()
        .name("minnal-desktop-load-model-catalog".to_string())
        .spawn(move || {
            if let Err(error) =
...
pub fn spawn_set_model(
    ...
    std::thread::Builder::new()
        .name("minnal-desktop-set-model".to_string())
        .spawn(move || {
            if let Err(error) =
...
fn socket_path() -> PathBuf {
    if let Ok(custom) = std::env::var("MINNAL_SOCKET") {
        return PathBuf::from(custom);
    }
    if let Ok(dir) = std::env::var("MINNAL_RUNTIME_DIR") {
        return PathBuf::from(dir).join("minnal.sock");
    }
    if let Ok(dir) = std::env::var("XDG_RUNTIME_DIR") {
        return PathBuf::from(dir).join("minnal.sock");
    }
    std::env::temp_dir()
        .join(format!("minnal-{}", runtime_user_discriminator()))
        .join("minnal.sock")
}
...
fn terminal_candidates(title: &str, minnal_args: &[&str]) -> Vec<Command> {
    let mut candidates = Vec::new();

    if let Ok(program) = std::env::var("MINNAL_DESKTOP_TERMINAL") {
        candidates.push(terminal_command(program, &[], minnal_args));
    }
    ...
    candidates.push(terminal_command("foot", &["-T", title, "--"], minnal_args));
    candidates.push(terminal_command("kitty", &["--title", title], minnal_args));
    candidates.push(terminal_command(
        "alacritty",
        &["-t", title, "-e"],
        minnal_args,
    ));
    candidates.push(terminal_command("wezterm", &["start", "--"], minnal_args));
    candidates.push(terminal_command(
        "x-terminal-emulator",
        &["-T", title, "-e"],
        minnal_args,
...
fn minnal_bin() -> String {
    std::env::var("MINNAL_BIN").unwrap_or_else(|_| "minnal".to_string())
}
...
mod tests {
    ...
    fn desktop_event_parser_maps_streaming_server_events() {
        ...
        assert_eq!(
            desktop_event_from_server_value(&json!({
                "type": "reloading",
                "new_socket": "/tmp/minnal-new.sock"
            })),
            Some(DesktopSessionEvent::Reloading {
                new_socket: Some("/tmp/minnal-new.sock".to_string())
            })
        );
        assert_eq!(
            desktop_event_from_server_value(&json!({
    ...
    fn desktop_worker_roundtrips_message_with_fake_server() -> Result<()> {
        ...
        let socket_path = std::env::temp_dir().join(format!(
            "minnal-desktop-worker-smoke-{}-{}.sock",
            std::process::id(),
        ...
        let previous_socket = std::env::var_os("MINNAL_SOCKET");
        unsafe {
            std::env::set_var("MINNAL_SOCKET", &socket_path);
        }
        ...
        restore_env_var("MINNAL_SOCKET", previous_socket);
        let _ = std::fs::remove_file(&socket_path);
    ...
    fn desktop_worker_emits_reloaded_before_real_done_after_fake_reload() -> Result<()> {
        ...
        let nonce = std::time::SystemTime::now()
            ...
            .as_nanos();
        let socket_path = std::env::temp_dir().join(format!(
            "minnal-desktop-worker-reload-old-{}-{nonce}.sock",
            std::process::id(),
        ));
        let new_socket_path = std::env::temp_dir().join(format!(
            "minnal-desktop-worker-reload-new-{}-{nonce}.sock",
            std::process::id(),
        ));
        let _ = std::fs::remove_file(&socket_path);
