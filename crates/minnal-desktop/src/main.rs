                    ...
                    if event.state == ElementState::Pressed =>
                {
                    ...
                    match app.handle_key(key_input) {
                        ...
                        KeyOutcome::OpenSession { session_id, title } => {
                            ...
                            {
                                eprintln!(
                                    "minnal-desktop: failed to open session {session_id}: {error:#}"
                                );
                            }
                        }
                        KeyOutcome::SpawnSession => {
                            ...
                            if let Err(error) = session_launch::launch_new_session() {
                                eprintln!("minnal-desktop: failed to spawn session: {error:#}");
                            } else {
                                spawn_session_cards_load(
                                    DesktopSessionCardsPurpose::WorkspaceRefresh,
                                    event_loop_proxy.clone(),
                        ...
                        KeyOutcome::SendDraft {
                            ...
                            if app.is_single_session() {
                                ...
                                match session_launch::spawn_message_to_session(
                                    ...
                                    Err(error) => eprintln!(
                                        "minnal-desktop: failed to send image draft to {session_id}: {error:#}"
                                    ),
                                }
                                ...
                                eprintln!(
                                    "minnal-desktop: failed to send draft to {session_id}: {error:#}"
                                );
                            } else {
                                spawn_session_cards_load(
                                    DesktopSessionCardsPurpose::WorkspaceRefresh,
            ...
            Event::AboutToWait => {
                ...
                if let Some(relaunch) = hot_reloader.poll() {
                    if let Err(error) = relaunch.spawn() {
                        eprintln!("minnal-desktop: failed to hot reload desktop: {error:#}");
                    } else {
                        target.exit();
                        return;
                    }
...
fn load_session_cards_for_desktop() -> Vec<workspace::SessionCard> {
    match session_data::load_recent_session_cards() {
        Ok(cards) => cards,
        Err(error) => {
            eprintln!("minnal-desktop: failed to load session metadata: {error:#}");
            Vec::new()
        }
    }
}
...
fn load_crashed_session_cards_for_desktop() -> Vec<workspace::SessionCard> {
    match session_data::load_crashed_session_cards() {
        Ok(cards) => cards,
        Err(error) => {
            eprintln!("minnal-desktop: failed to load crashed session metadata: {error:#}");
            Vec::new()
        }
    }
}
...
fn spawn_recovery_session_count_scan(
    ...
    if let Err(error) = std::thread::Builder::new()
        .name("minnal-desktop-recovery-scan".to_string())
        .spawn(move || {
    ...
    {
        eprintln!("minnal-desktop: failed to start recovery scan: {error:#}");
    }
}
...
fn spawn_single_session_card_refresh(
    ...
    if let Err(error) = std::thread::Builder::new()
        .name("minnal-desktop-session-card-refresh".to_string())
        .spawn(move || {
    ...
    {
        eprintln!("minnal-desktop: failed to start session card refresh: {error:#}");
    }
}
...
fn spawn_session_cards_load(
    ...
    if let Err(error) = std::thread::Builder::new()
        .name(format!("minnal-desktop-session-cards-{purpose:?}"))
        .spawn(move || {
            if !delay.is_zero() {
    ...
    {
        eprintln!("minnal-desktop: failed to start session card load: {error:#}");
    }
}

fn spawn_restore_crashed_sessions(event_loop_proxy: EventLoopProxy<DesktopUserEvent>) {
    if let Err(error) = std::thread::Builder::new()
        .name("minnal-desktop-restore-crashed-sessions".to_string())
        .spawn(move || {
            let started = Instant::now();
    ...
    {
        eprintln!("minnal-desktop: failed to start crashed-session restore: {error:#}");
    }
}
...
fn spawn_desktop_preferences_saver() -> Option<mpsc::Sender<workspace::DesktopPreferences>> {
    ...
    match std::thread::Builder::new()
        .name("minnal-desktop-preferences-saver".to_string())
        .spawn(move || {
        ...
        Ok(_) => Some(tx),
        Err(error) => {
            eprintln!("minnal-desktop: failed to start preferences saver: {error:#}");
            None
        }
    }
}
...
fn queue_desktop_preferences_save(
    ...
    if let Err(error) = std::thread::Builder::new()
        .name("minnal-desktop-preferences-save-once".to_string())
        .spawn(move || {
            save_desktop_preferences_off_ui_thread(preferences, 1, Duration::ZERO);
        })
    {
        eprintln!("minnal-desktop: failed to queue preferences save: {error:#}");
    }
}
...
fn startup_log_requested(args: &[String]) -> bool {
    args.iter().any(|arg| arg == "--startup-log")
        || std::env::var_os("MINNAL_DESKTOP_STARTUP_LOG").is_some_and(env_flag_enabled)
}
...
async fn render_hero_frame_to_image(
    ...
    let (device, queue) = adapter
        .request_device(
            &wgpu::DeviceDescriptor {
                label: Some("minnal-desktop-hero-capture-device"),
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("minnal-desktop-hero-capture-primitive-shader"),
        source: wgpu::ShaderSource::Wgsl(SHADER.into()),
    });
    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("minnal-desktop-hero-capture-pipeline-layout"),
        bind_group_layouts: &[],
        push_constant_ranges: &[],
    });
    let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: Some("minnal-desktop-hero-capture-primitive-pipeline"),
    ...
    let texture = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("minnal-desktop-hero-capture-texture"),
        size: wgpu::Extent3d {
            width: size.width,
            height: size.height,
    ...
    let vertex_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("minnal-desktop-hero-capture-vertices"),
    ...
    let output_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("minnal-desktop-hero-capture-readback"),
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("minnal-desktop-hero-capture-encoder"),
    });
    {
        let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("minnal-desktop-hero-capture-pass"),
...
impl DesktopStartupTrace {
    ...
    fn mark(&self, milestone: &str) {
        if self.enabled {
            eprintln!(
                "minnal-desktop startup +{:>7.2} ms  {milestone}",
...
fn spawn_session_event_forwarder(
    ...
    if let Err(error) = std::thread::Builder::new()
        .name("minnal-desktop-session-event-forwarder".to_string())
        .spawn(move || {
    ...
    {
        eprintln!("minnal-desktop: failed to start session event forwarder: {error:#}");
    }
}
    ...
    println!(
        ...
        serde_json::to_string_pretty(&serde_json::json!({
            ...
            "no_paint_watchdog": {
                ...
                "log_event": "minnal-desktop-no-paint-profile",
...
fn prewarm_desktop_text_renderer(
    ...
    if let Err(error) = text_renderer.prepare(
        ...
        eprintln!("minnal-desktop: failed to prewarm text renderer: {error:?}");
    }
}
...
fn load_desktop_preferences() -> Option<workspace::DesktopPreferences> {
    match desktop_prefs::load_preferences() {
        ...
        Err(error) => {
            eprintln!("minnal-desktop: failed to load desktop preferences: {error:#}");
            None
        }
    }
}
...
fn log_desktop_session_event_batch_profile(
    ...
    emit_desktop_profile_event(
        "minnal-desktop-session-event-profile",
        serde_json::json!({
...
fn log_desktop_session_card_refresh_profile(
    ...
    emit_desktop_profile_event(
        "minnal-desktop-session-card-refresh-profile",
        serde_json::json!({
            "session_id": session_id,
...
fn log_desktop_session_cards_load_profile(
    ...
    emit_desktop_profile_event(
        "minnal-desktop-session-cards-load-profile",
        serde_json::json!({
...
fn log_desktop_preferences_save_profile(
    ...
    emit_desktop_profile_event(
        "minnal-desktop-preferences-save-profile",
        serde_json::json!({
...
fn log_desktop_crashed_sessions_restore_profile(restored: usize, errors: usize, elapsed: Duration) {
    ...
    emit_desktop_profile_event(
        "minnal-desktop-crashed-sessions-restore-profile",
        serde_json::json!({
            "restored": restored,
            "errors": errors,
...
impl DesktopFrameProfiler {
    fn new() -> Self {
        let mode = std::env::var("MINNAL_DESKTOP_FRAME_PROFILE").ok();
        let enabled = !matches!(mode.as_deref(), Some("0" | "false" | "off"));
        let log_all = matches!(mode.as_deref(), Some("all" | "trace"));
        let budget = std::env::var("MINNAL_DESKTOP_FRAME_BUDGET_MS")
            .ok()
...
impl DesktopInteractionLatencyProfiler {
    fn new() -> Self {
        ...
        let enabled = !matches!(mode.as_deref(), Some("0" | "false" | "off"));
        let log_all = matches!(mode.as_deref(), Some("all" | "trace"));
        let budget = std::env::var("MINNAL_DESKTOP_INPUT_LATENCY_BUDGET_MS")
            .ok()
            .and_then(|value| value.parse::<f64>().ok())
            .filter(|value| value.is_finite() && *value > 0.0)
            .map(|ms| Duration::from_secs_f64(ms / 1000.0))
    ...
    fn observe_presented(&mut self, frame: &DesktopRenderFrameResult) {
        ...
        eprintln!(
            "minnal-desktop-latency-profile {}",
            serde_json::json!({
                "kind": pending.kind,
...
impl DesktopNoPaintWatchdog {
    ...
    fn new_with_start(now: Instant) -> Self {
        ...
        let enabled = !matches!(mode.as_deref(), Some("0" | "false" | "off"));
        let log_all = matches!(mode.as_deref(), Some("all" | "trace"));
        let budget = std::env::var("MINNAL_DESKTOP_NO_PAINT_BUDGET_MS")
            .ok()
            .and_then(|value| value.parse::<f64>().ok())
            .filter(|value| value.is_finite() && *value > 0.0)
            .map(|ms| Duration::from_secs_f64(ms / 1000.0))
    ...
    fn observe_active_tick(&mut self, now: Instant, context: NoPaintWatchdogContext) -> bool {
        ...
        if report_due {
            ...
            emit_desktop_profile_event(
                "minnal-desktop-no-paint-profile",
                serde_json::json!({
...
fn desktop_profile_log_path() -> Option<PathBuf> {
    if std::env::var_os("MINNAL_DESKTOP_PROFILE_LOG").is_some_and(|value| !env_flag_enabled(value))
    {
        return None;
    }
    if let Some(path) = std::env::var_os("MINNAL_DESKTOP_PROFILE_LOG_PATH") {
        if path.is_empty() {
            return None;
        }
...
fn desktop_profile_stderr_enabled() -> bool {
    std::env::var_os("MINNAL_DESKTOP_PROFILE_STDERR").is_none_or(env_flag_enabled)
}
...
fn desktop_profile_log_sender() -> Option<&'static mpsc::Sender<DesktopProfileLogLine>> {
    DESKTOP_PROFILE_LOG_TX
        .get_or_init(|| {
            ...
            match std::thread::Builder::new()
                .name("minnal-desktop-profile-log".to_string())
                .spawn(move || {
                ...
                Ok(_) => Some(tx),
                Err(error) => {
                    eprintln!("minnal-desktop: failed to start profile logger: {error:#}");
                    None
                }
            }
        })
...
fn log_desktop_slow_interaction(
    ...
    let enabled = std::env::var("MINNAL_DESKTOP_FRAME_PROFILE")
        .ok()
    ...
    emit_desktop_profile_event(
        "minnal-desktop-interaction-profile",
        serde_json::json!({
            "kind": kind,
...
impl<'window> Canvas<'window> {
    async fn new(window: &'window Window, startup_trace: DesktopStartupTrace) -> Result<Self> {
        ...
        let (device, queue) = adapter
            .request_device(
                &wgpu::DeviceDescriptor {
                    label: Some("minnal-desktop-device"),
                    required_features: wgpu::Features::empty(),
                    required_limits: wgpu::Limits::default(),
                },
                None,
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("minnal-desktop-primitive-shader"),
            source: wgpu::ShaderSource::Wgsl(SHADER.into()),
        });
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("minnal-desktop-primitive-pipeline-layout"),
            bind_group_layouts: &[],
            push_constant_ranges: &[],
        });
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("minnal-desktop-primitive-pipeline"),
            layout: Some(&pipeline_layout),
            vertex: wgpu::VertexState {
                module: &shader,
                entry_point: "vs_main",
        ...
        if self.primitive_vertex_capacity < vertices.len() {
            ...
            self.primitive_vertex_buffer =
                Some(self.device.create_buffer(&wgpu::BufferDescriptor {
                    label: Some("minnal-desktop-workspace-vertices"),
                    size,
                    usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
                    mapped_at_creation: false,
                }));
    ...
    fn render(
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("minnal-desktop-render-workspace"),
            });
        let now = Instant::now();
        ...
        if self.text_needs_prepare {
            ...
            if text_areas.is_empty() {
                ...
                if let Err(error) = text_renderer.prepare(
                    ...
                    eprintln!("minnal-desktop: failed to prepare text: {error:?}");
                } else {
                    self.text_needs_prepare = false;
                }
            }
        ...
        if self.streaming_text_needs_prepare {
            ...
            if streaming_text_areas.is_empty() {
                ...
                if let Err(error) = text_renderer.prepare(
                    ...
                    eprintln!("minnal-desktop: failed to prepare streaming text: {error:?}");
                } else {
                    self.streaming_text_needs_prepare = false;
                }
            }
        ...
        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("minnal-desktop-workspace-pass"),
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    view: &view,
                    resolve_target: None,
                    ops: wgpu::Operations {
            ...
            {
                eprintln!("minnal-desktop: failed to render text: {error:?}");
            }
            if has_streaming_text_buffer
                ...
                && let Err(error) = text_renderer.render(text_atlas, &mut render_pass)
            {
                eprintln!("minnal-desktop: failed to render streaming text: {error:?}");
            }
        }
...
fn desktop_build_hash_label() -> &'static str {
    option_env!("MINNAL_DESKTOP_GIT_HASH").unwrap_or("unknown")
}
