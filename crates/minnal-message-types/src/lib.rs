...
impl ToolDefinition {
    ...
    ///
    /// This uses minnal's standard chars/4 heuristic, matching other token
...
pub enum ContentBlock {
    ...
    /// compaction state across turns/saves when minnal explicitly triggers it.
    OpenAICompaction {
...
pub enum StreamEvent {
    ...
    /// Native tool call from a provider bridge that needs execution by minnal
    NativeToolCall {
        request_id: String,
        tool_name: String,
        input: serde_json::Value,
