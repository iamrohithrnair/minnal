...
fn with_temp_minnal_home<T>(f: impl FnOnce() -> T) -> T {
    ...
    minnal_core::env::set_var("MINNAL_HOME", temp_home.path());
    let result = f();
    if let Some(prev_home) = prev_home {
        minnal_core::env::set_var("MINNAL_HOME", prev_home);
    } else {
        minnal_core::env::remove_var("MINNAL_HOME");
    }
    result
}
...
fn test_binary_version_hash_mismatch_rejects_publish_candidate() {
    ...
    let error =
        validate_binary_version_matches_source_report(&report, Path::new("minnal"), &source)
...
fn test_find_repo_in_ancestors_walks_upward() {
    let temp = tempfile::tempdir().expect("tempdir");
    let repo = temp.path().join("minnal-repo");
...
#[test]
fn launcher_dir_uses_sandbox_bin_when_minnal_home_is_set() {
    with_temp_minnal_home(|| {
        let launcher_dir = launcher_dir().expect("launcher dir");
        let expected = storage::minnal_dir().expect("minnal dir").join("bin");
...
fn update_launcher_symlink_stays_inside_sandbox_home() {
    with_temp_minnal_home(|| {
        ...
        let expected_launcher = storage::minnal_dir()
            .expect("minnal dir")
            .join("bin")
            .join(binary_name());
