...
use minnal_storage as storage;
use std::path::{Path, PathBuf};
use std::process::Command;
use std::time::SystemTime;

/// Get the minnal repository directory
...
pub fn selfdev_build_command_for_target(
    ...
    let specs = match target {
        SelfDevBuildTarget::Tui => vec![("minnal", "minnal")],
        SelfDevBuildTarget::Desktop => vec![("minnal-desktop", "minnal-desktop")],
        SelfDevBuildTarget::All | SelfDevBuildTarget::Auto => {
            vec![("minnal", "minnal"), ("minnal-desktop", "minnal-desktop")]
        }
    };
...
fn infer_selfdev_build_target(repo_dir: &Path) -> SelfDevBuildTarget {
    ...
    for line in text.lines() {
        ...
        if path == "Cargo.toml" || path == "Cargo.lock" || path.starts_with(".cargo/") {
            desktop = true;
            other = true;
        } else if path.starts_with("crates/minnal-desktop/") {
            desktop = true;
...
///
/// Defaults to `~/.local/bin` on Unix, `%LOCALAPPDATA%\minnal\bin` on Windows.
/// Overridable with `MINNAL_INSTALL_DIR`.
pub fn launcher_dir() -> Result<PathBuf> {
    if let Some(custom) = non_empty_env_path("MINNAL_INSTALL_DIR") {
        return Ok(custom);
    }

    if let Some(sandbox_home) = non_empty_env_path("MINNAL_HOME") {
    ...
    #[cfg(windows)]
    {
        if let Ok(local) = std::env::var("LOCALAPPDATA") {
            return Ok(PathBuf::from(local).join("minnal").join("bin"));
        }
        Ok(home_dir()?
            .join("AppData")
            .join("Local")
...
/// Check if a directory is the minnal repository
pub fn is_minnal_repo(dir: &Path) -> bool {
    // Check for Cargo.toml with name = "minnal"
...
mod tests {
    ...
    fn repo_fixture(git_file: bool) -> tempfile::TempDir {
        ...
        if git_file {
            std::fs::write(temp.path().join(".git"), "gitdir: /tmp/minnal-test-git\n")
                .expect("git file");
        } else {
    ...
    fn find_repo_in_ancestors_finds_workspace_from_crate_dir() {
        ...
        let crate_dir = repo.path().join("crates").join("minnal-build-support");
    ...
    #[test]
    fn is_minnal_repo_accepts_git_file_for_worktree() {
        let repo = repo_fixture(true);
        assert!(is_minnal_repo(repo.path()));
    }
}
