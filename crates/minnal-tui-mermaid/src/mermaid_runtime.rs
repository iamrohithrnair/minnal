...
fn picker_init_mode_from_env() -> PickerInitMode {
    picker_init_mode_from_probe_env(std::env::var("MINNAL_MERMAID_PICKER_PROBE").ok().as_deref())
}
...
fn prewarm_svg_font_db_async() {
    SVG_FONT_DB_PREWARM_STARTED.get_or_init(|| {
        let _ = std::thread::Builder::new()
            .name("minnal-mermaid-fontdb-prewarm".to_string())
            .spawn(|| {
                let _ = &*SVG_FONT_DB;
            });
    });
...
/// Set MINNAL_MERMAID_PICKER_PROBE=1 to force full stdio capability probing.
