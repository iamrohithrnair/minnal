...
pub trait Provider: Send + Sync {
    ...
    /// Returns true if minnal should use its own compaction for this provider.
    fn supports_compaction(&self) -> bool {
        false
    }

    /// Returns true if minnal should proactively run its own summary-based compaction.
...
/// Native tool result to send back to provider bridges that delegate tool execution to minnal.
...
/// Canonical User-Agent for generic outbound Minnal HTTP requests.
pub const MINNAL_USER_AGENT: &str = concat!("minnal/", env!("CARGO_PKG_VERSION"));
    ...
    CLIENT
        .get_or_init(|| {
            reqwest::Client::builder()
                ...
                .build()
                .unwrap_or_else(|err| {
                    eprintln!("minnal: failed to build shared provider HTTP client: {err}");
                    reqwest::Client::builder()
                        .user_agent(MINNAL_USER_AGENT)
                        .build()
                        .expect("fallback Minnal HTTP client should build")
                })
        })
        .clone()
}
...
mod tests {
    ...
    #[test]
    fn canonical_user_agent_identifies_minnal() {
        assert!(MINNAL_USER_AGENT.starts_with("minnal/"));
    }
}
