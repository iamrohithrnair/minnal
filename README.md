...
minnal is built to be as performant and resource efficient as possible. Every metric is optimized to the bone, which is important for scaling multi-session workflows. Here we sample a few metrics to show the difference: RAM usage and boot up.
  ...
  <tr>
    <td valign="top" align="center" width="50%">
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td><strong>minnal (local embedding off)</strong></td>
            <td align="right"><strong>27.8 MB</strong></td>
            <td align="right">baseline</td>
          </tr>
          <tr>
            <td><strong>minnal</strong></td>
...
|---|---:|---:|---:|
| **minnal** | **14.0 ms** | 10.1–19.3 ms | baseline |
...
|---|---:|---:|---:|
| **minnal** | **48.7 ms** | 30.3–62.7 ms | baseline |
...
|---|---:|---:|
| **minnal (local embedding off)** | **~9.9 MB** | baseline |
| **minnal** | **~10.4 MB** | **1.1× more RAM** |
...
The side panel is a place for auxiliary information. Tell your minnal agent to load a file into the side panel and see it update in real time, or tell your agent to write directly to the side panel, or use it as a diff viewer. The side panel (and chat) is able to render mermaid diagrams inline.
...
Minnal can render at over a thousand fps. Your monitor will not have the refresh rate to show you, but this means you will not have silly flicker problems.

The custom scrollback implementation of minnal allows it to do much more than a native scrollback. However, it is a terminal-level limitation that I cannot have smooth, partial line scrolling with a custom scrollback. To fix this, I made my own terminal. Handterm https://github.com/1jehuang/handterm implements a native scroll api, and also happens to be very effiecent. This is a work in progress. Scrolling is still well implemented for normal terminals.

Minnal is left-aligned by default. You can switch to centered mode with the `Alt+C` hotkey, with the `/alignment` command, or in the config.
...
minnal works with subscription-backed OAuth flows and many provider integrations, so you can use the models you already pay for and still fall back to direct API providers when needed.
...
For custom OpenAI-compatible endpoints, minnal now prompts for the API base and supports local localhost servers without requiring an API key.

### Config-file setup for self-hosted endpoints and MCP

If you prefer to configure things by editing files instead of using the login UI, minnal supports both a custom OpenAI-compatible endpoint config and MCP config files.

#### Self-hosted OpenAI-compatible endpoints, including vLLM

For agents and scripts, the preferred path is the one-shot provider profile command. It writes a named profile to `~/.minnal/config.toml`, stores secrets in minnal's private app config directory when requested, and prints exact run/validation commands:
...
minnal --provider-profile my-api auth-test --prompt 'Reply exactly MINNAL_PROVIDER_SETUP_OK'
...
The generated profile can also be edited manually in `~/.minnal/config.toml`:
...
api_key_env = "MINNAL_PROVIDER_MY_API_API_KEY"
...
The custom OpenAI-compatible provider reads overrides from environment variables or from an env file in minnal's app config directory. On Linux this is usually `~/.config/minnal/`, so the default file is usually:

```text
~/.config/minnal/openai-compatible.env
```

Example for a local or LAN vLLM server:

```bash
MINNAL_OPENAI_COMPAT_API_BASE=http://192.168.1.50:8000/v1
MINNAL_OPENAI_COMPAT_DEFAULT_MODEL=Qwen/Qwen3-Coder-30B-A3B-Instruct
...
- `~/.minnal/mcp.json` for global MCP servers
- `.minnal/mcp.json` for project-local MCP servers
...
On first run, minnal also tries to import MCP servers from `~/.claude/mcp.json` and `~/.codex/config.toml` if `~/.minnal/mcp.json` does not exist yet.

For headless or SSH sessions, OAuth-style providers support `minnal login --provider <provider> --no-browser` (alias: `--headless`) so minnal prints the auth URL/QR and falls back to manual code or callback paste instead of trying to launch a local browser.
...
Pending scriptable login state is stored under `~/.minnal/pending-login/`, automatically expires, and stale entries are cleaned up when new scriptable logins start or resume.

For the built-in OpenAI login flow, minnal opens a local callback on
...
Minnal also supports easy multi-account switching. Ran out of tokens on your first ChatGPT Pro subscription? /account and quickly switch to your second.

---

## Customizability / Self-Dev

Minnal is inventing a new form of customizability. One that doesn't limit you to what a plugin or extension can do. Tell your minnal agent to enter self dev mode, and it will start modifying its own source code. Minnal is optimized to iterate on itself. There is significant infrastructure around self developement, which allows it to edit, build, and test its own source code, then reload its own binary and continue work in your (potentially many) sessions, fully automatically.

It is reccomended that you use a frontier model for this. The minnal codebase is not a simple one, and weaker models can make subtle, breaking changes. GPT 5.5 or the latest available frontier model works well.
...
The devil is in the details. There are many undocumented optimizations and niceties that minnal implements. Some examples:
...
minnal comes with instructions on how to set up Firefox Agent Bridge. Ask you agent to set it up, and then you will have browser automation in minnal as well.

Agent grep is a grep tool I made for the minnal agent. It adds file strucuture information (ie the list of functions, their displacement, etc) to the grep return, so that the agent can infer more of what the file doesn without actually reading the file. It also implements a harness-level integration that adaptively truncates returns based on what the agent has already seen. This saves on context a lot.
...
Resume sessions from different harnesses. Claude code broke on you? Resume the session from minnal and continue where you left off. Session resume is supported for codex, claude code, opencode, and pi.
...
A native iOS application version of minnal is coming soon. This will allow you to work with minnal on your personal machine's environment from your phone, via Tailscale. Openclaw like features will be bundled with this iOS application.
...
minnal supports interactive TUI use, non-interactive runs, persistent server/client workflows,
...
minnal includes a first-class built-in `browser` tool for browser control inside agent sessions.
...
1. Detect the operating system, available package managers, and shell environment, then install minnal using the best matching command below instead of referring me somewhere else:
   ...
   - For local self-dev / refactor work on Linux x86_64, prefer:
     scripts/dev_cargo.sh build --release -p minnal --bin minnal
...
4. Before attempting any interactive login flow, assess which providers are already available non-interactively and prefer those first. Check existing local credentials, config files, CLI sessions, and environment variables such as:
   - Claude: `~/.minnal/auth.json`, `~/.claude/.credentials.json`, `~/.local/share/opencode/auth.json`, `ANTHROPIC_API_KEY`
   - OpenAI: `~/.minnal/openai-auth.json`, `~/.codex/auth.json`, `OPENAI_API_KEY`
   - Gemini: `~/.minnal/gemini_oauth.json`, `~/.gemini/oauth_creds.json`
   - GitHub Copilot: existing auth under `~/.config/github-copilot/`
   - Azure OpenAI: `~/.config/minnal/azure-openai.env`, `AZURE_OPENAI_*`, or an existing `az login`
   - OpenRouter: `OPENROUTER_API_KEY`
   - Fireworks: `~/.config/minnal/fireworks.env`, `FIREWORKS_API_KEY`
   - MiniMax: `~/.config/minnal/minimax.env`, `MINIMAX_API_KEY`
   - Alibaba Cloud Coding Plan: existing minnal config/env if present
...
This is intended to be a copy-paste bootstrap prompt for minnal itself or any other coding agent.
