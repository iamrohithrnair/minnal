...
SOCKET_PATH = os.path.join(RUNTIME_DIR, "minnal-debug.sock")
MINNAL_DIR = os.path.expanduser("~/.minnal")
...
def test_selfdev_status():
    ...
    try:
        ...
        result = send_cmd(sock, "create_session:selfdev:/home/jeremy/minnal")
...
def test_reload_context():
    ...
    context_candidates = sorted(
        glob.glob(os.path.join(MINNAL_DIR, "reload-context-*.json"))
    )
    legacy_context_path = os.path.join(MINNAL_DIR, "reload-context.json")
