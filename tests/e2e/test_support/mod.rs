//! End-to-end tests for minnal using a mock provider
//!
...
pub(crate) use minnal::agent::Agent;
pub(crate) use minnal::message::{ContentBlock, Message, Role, StreamEvent, ToolDefinition};
pub(crate) use minnal::protocol::{Request, ServerEvent};
pub(crate) use minnal::provider::{EventStream, Provider};
pub(crate) use minnal::server;
pub(crate) use minnal::session::{Session, StoredCompactionState};
pub(crate) use minnal::tool::Registry;
...
static MINNAL_HOME_LOCK: std::sync::OnceLock<Mutex<()>> = std::sync::OnceLock::new();
...
fn lock_minnal_home() -> std::sync::MutexGuard<'static, ()> {
    let mutex = MINNAL_HOME_LOCK.get_or_init(|| Mutex::new(()));
...
impl TestEnvGuard {
    pub(crate) fn new() -> Result<Self> {
        let lock = lock_minnal_home();
        let temp_home = tempfile::Builder::new()
            .prefix("minnal-e2e-home-")
            .tempdir()?;
        let prev_home = std::env::var_os("MINNAL_HOME");
        let prev_runtime_dir = std::env::var_os("MINNAL_RUNTIME_DIR");
        let prev_test_session = std::env::var_os("MINNAL_TEST_SESSION");
        let prev_debug_control = std::env::var_os("MINNAL_DEBUG_CONTROL");
        let runtime_dir = temp_home.path().join("runtime");
        std::fs::create_dir_all(&runtime_dir)?;

        minnal::env::set_var("MINNAL_HOME", temp_home.path());
        minnal::env::set_var("MINNAL_RUNTIME_DIR", &runtime_dir);
        minnal::env::set_var("MINNAL_TEST_SESSION", "1");
        minnal::env::set_var("MINNAL_DEBUG_CONTROL", "1");
...
impl Drop for TestEnvGuard {
    fn drop(&mut self) {
        if let Some(prev_home) = &self.prev_home {
            minnal::env::set_var("MINNAL_HOME", prev_home);
        } else {
            minnal::env::remove_var("MINNAL_HOME");
        }

        if let Some(prev_runtime_dir) = &self.prev_runtime_dir {
            minnal::env::set_var("MINNAL_RUNTIME_DIR", prev_runtime_dir);
        } else {
            minnal::env::remove_var("MINNAL_RUNTIME_DIR");
        }

        if let Some(prev_test_session) = &self.prev_test_session {
            minnal::env::set_var("MINNAL_TEST_SESSION", prev_test_session);
        } else {
            minnal::env::remove_var("MINNAL_TEST_SESSION");
        }

        if let Some(prev_debug_control) = &self.prev_debug_control {
            minnal::env::set_var("MINNAL_DEBUG_CONTROL", prev_debug_control);
        } else {
            minnal::env::remove_var("MINNAL_DEBUG_CONTROL");
        }
    }
}
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(prev) = &self.prev {
            minnal::env::set_var(self.name, prev);
        } else {
            minnal::env::remove_var(self.name);
        }
    }
}
...
fn pair_test_device(token: &str) -> Result<()> {
    let mut registry = minnal::gateway::DeviceRegistry::load();
    ...
    registry.devices.push(minnal::gateway::PairedDevice {
...
pub(crate) async fn run_unix_transport_scenario() -> Result<TransportScenarioResult> {
    ...
    let provider: Arc<dyn minnal::provider::Provider> = Arc::new(provider);
    let server_instance =
    ...
    let result = async {
        ...
        if !saw_message_done {
            ...
            if !response_persisted {
                ...
                let logs = std::env::var_os("MINNAL_HOME")
...
pub(crate) async fn run_websocket_transport_scenario() -> Result<TransportScenarioResult> {
    ...
    let server_instance =
        server::Server::new_with_paths(provider, socket_path.clone(), debug_socket_path.clone())
            .with_gateway_config(minnal::gateway::GatewayConfig {
                port: gateway_port,
...
pub(crate) async fn wait_for_selfdev_reload_cycle(
    ...
    while Instant::now() < deadline {
        let marker_active = minnal::server::reload_marker_active(Duration::from_secs(30));
