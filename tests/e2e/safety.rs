...
fn test_safety_classification() {
    use minnal::safety::SafetySystem;

    let safety = SafetySystem::new();

    // Tier 1: auto-allowed
    assert!(safety.classify("read") == minnal::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("glob") == minnal::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("grep") == minnal::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("memory") == minnal::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("todoread") == minnal::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("todowrite") == minnal::safety::ActionTier::AutoAllowed);

    // Tier 2: requires permission
    assert!(safety.classify("bash") == minnal::safety::ActionTier::RequiresPermission);
    assert!(safety.classify("edit") == minnal::safety::ActionTier::RequiresPermission);
    assert!(safety.classify("write") == minnal::safety::ActionTier::RequiresPermission);
    assert!(
        safety.classify("create_pull_request") == minnal::safety::ActionTier::RequiresPermission
    );
    assert!(safety.classify("send_email") == minnal::safety::ActionTier::RequiresPermission);
...
fn test_safety_permission_flow() {
    use minnal::safety::{PermissionRequest, PermissionResult, SafetySystem, Urgency};
...
fn test_safety_transcript() {
    use minnal::safety::{AmbientTranscript, SafetySystem, TranscriptStatus};
...
fn test_safety_summary_generation() {
    use minnal::safety::{ActionLog, ActionTier, SafetySystem};
