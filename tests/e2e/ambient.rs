...
fn test_ambient_state_lifecycle() {
    use minnal::ambient::{AmbientCycleResult, AmbientState, AmbientStatus, CycleStatus};
...
fn test_ambient_scheduled_queue() {
    use minnal::ambient::{Priority, ScheduledItem, ScheduledQueue};

    let tmp = std::env::temp_dir().join("minnal-test-queue.json");
    ...
    queue.push(ScheduledItem {
        ...
        target: minnal::ambient::ScheduleTarget::Ambient,
...
fn test_adaptive_scheduler_intervals() {
    use minnal::ambient_scheduler::{AdaptiveScheduler, AmbientSchedulerConfig};
...
async fn test_ambient_end_cycle_tool() -> Result<()> {
    ...
    let result = minnal::tool::ambient::take_cycle_result();
...
async fn test_ambient_request_permission_tool() -> Result<()> {
    ...
    minnal::tool::ambient::register_ambient_session(ambient_session_id.clone());

    let response = agent.run_once_capture("Request permission").await?;
    minnal::tool::ambient::unregister_ambient_session(&ambient_session_id);
...
async fn test_ambient_runner_status() {
    use minnal::ambient_runner::AmbientRunnerHandle;
...
async fn test_ambient_runner_trigger_and_stop() {
    use minnal::ambient::AmbientStatus;
    use minnal::ambient_runner::AmbientRunnerHandle;
    use minnal::safety::SafetySystem;
...
fn test_memory_reinforcement_provenance() {
    use minnal::memory::{MemoryCategory, MemoryEntry};
...
fn test_ambient_config_defaults() {
    use minnal::config::AmbientConfig;
...
fn test_ambient_lock() {
    use minnal::ambient::AmbientLock;
    let _env = setup_test_env().expect("failed to setup isolated MINNAL_HOME");
...
async fn test_full_ambient_cycle_simulation() -> Result<()> {
    ...
    agent.set_system_prompt("You are the minnal ambient maintenance agent.");
