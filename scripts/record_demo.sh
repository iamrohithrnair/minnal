#!/bin/bash
# minnal demo recording orchestrator
# Usage: ./scripts/record_demo.sh <demo_name> <prompt>
#
# This script:
# 1. Opens a fresh minnal in a new kitty window
# 2. Starts wf-recorder on that window
# 3. Sends the prompt to minnal
...
DEMO_DIR="/tmp/minnal-demo/$DEMO_NAME"
OUTPUT_DIR="$repo_root/assets/demos"
SOCK=$(ls /tmp/kitty.sock* 2>/dev/null | head -1)

mkdir -p "$DEMO_DIR" "$OUTPUT_DIR"

echo "=== minnal Demo Recorder ==="
echo "Demo: $DEMO_NAME"
echo "Prompt: $PROMPT"
echo "Working dir: $DEMO_DIR"
echo ""

# Step 1: Launch minnal in a new kitty OS window
echo "[1/5] Launching minnal..."
kitten @ --to unix:$SOCK launch --type=os-window \
    --cwd "$DEMO_DIR" \
    --title "minnal-demo-$DEMO_NAME" \
    "$repo_root/target/release/minnal"

sleep 3  # Let minnal fully start

# Step 2: Find the window
DEMO_WIN_ID=$(niri msg windows 2>/dev/null | grep -B5 "minnal-demo-$DEMO_NAME" | grep "Window ID" | awk '{print $3}' | tr -d ':')
...
# Step 4: Type the prompt into minnal
...
for os_win in data:
    for tab in os_win.get('tabs', []):
        for win in tab.get('windows', []):
            if 'minnal-demo-$DEMO_NAME' in win.get('title', ''):
                print(win['id'])
                sys.exit(0)
")
...
if [ -n "$KITTY_WIN_ID" ]; then
    ...
    kitten @ --to unix:$SOCK send-text --match "title:minnal-demo-$DEMO_NAME" "$PROMPT"
    sleep 0.5
    kitten @ --to unix:$SOCK send-text --match "title:minnal-demo-$DEMO_NAME" $'\r'
fi
...
# Poll for completion - check if minnal is still processing
