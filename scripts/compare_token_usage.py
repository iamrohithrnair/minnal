#!/usr/bin/env python3
"""
Compare token usage between minnal and Claude Code CLI.

This script runs the same prompts through both tools and compares their token usage.
The goal is to verify that minnal's token consumption is within expected bounds
compared to the official Claude Code CLI.

NOTE: minnal typically uses FEWER tokens than Claude CLI because:
1. minnal has a smaller/simpler system prompt
2. minnal registers fewer tools (Claude CLI has many built-in tools)
3. Different prompt caching behavior

The test PASSES if minnal uses fewer tokens OR at most 50% more tokens.
...
Requirements:
    - minnal built and in PATH or at target/release/minnal
...
def find_minnal_binary() -> str:
    """Find the minnal binary."""
    # Check target/release first
    repo_root = Path(__file__).parent.parent
    release_binary = repo_root / "target" / "release" / "minnal"
    if release_binary.exists():
        return str(release_binary)

    # Check PATH
    result = subprocess.run(["which", "minnal"], capture_output=True, text=True)
    if result.returncode == 0:
        return result.stdout.strip()

    raise FileNotFoundError("minnal binary not found. Run 'cargo build --release' first.")
...
def run_minnal(prompt: str, workdir: str, minnal_binary: str, model: str = "claude-opus-4-5-20251101") -> RunResult:
    """Run minnal and capture token usage from trace output."""
    try:
        # Create a temporary MINNAL_HOME to avoid polluting user's sessions
        with tempfile.TemporaryDirectory() as tmpdir:
            env = os.environ.copy()
            env["MINNAL_HOME"] = tmpdir
            env["MINNAL_TRACE"] = "1"
...
def compare_usage(claude_result: RunResult, minnal_result: RunResult, verbose: bool = False) -> dict:
    """Compare token usage between Claude CLI and minnal."""
    c = claude_result.usage
    j = minnal_result.usage
...
def print_comparison(comparison: dict, prompt: str, verbose: bool = False):
    ...
    print(f"\n{'Metric':<20} {'Claude':<15} {'minnal':<15} {'Diff':<15} {'% Diff':<10}")
    print("-" * 75)
...
def run_test_suite(verbose: bool = False, runs: int = 1) -> list:
    ...
    minnal_binary = find_minnal_binary()
    print(f"Using minnal binary: {minnal_binary}")
    ...
    with tempfile.TemporaryDirectory() as workdir:
        for prompt in prompts:
            for run_num in range(runs):
                ...
                print("  Running minnal...", end=" ", flush=True)
                minnal_result = run_minnal(prompt, workdir, minnal_binary)
                if minnal_result.success:
                    print(f"OK ({minnal_result.usage.total} tokens)")
                else:
                    print(f"FAILED: {minnal_result.error}")
                    if verbose:
                        print(f"    Output: {minnal_result.output[:200]}")

                if claude_result.success and minnal_result.success:
                    comparison = compare_usage(claude_result, minnal_result, verbose)
                    results.append({
                        "prompt": prompt,
                        "run": run_num + 1,
...
def summarize_results(results: list) -> bool:
    ...
    total_minnal = sum(r["comparison"]["minnal"]["total"] for r in results)
    total_diff = total_minnal - total_claude
    ...
    total_minnal_io = sum(
        r["comparison"]["minnal"]["input"] + r["comparison"]["minnal"]["output"]
        for r in results
    )

    if total_claude > 0:
        pct_diff = ((total_minnal - total_claude) / total_claude) * 100
    else:
        pct_diff = 0
    ...
    print(f"minnal:      {total_minnal}")
    print(f"Difference: {total_diff:+} ({pct_diff:+.1f}%)")

    print(f"\n--- Input + Output only (excluding cache) ---")
    print(f"Claude CLI: {total_claude_io}")
    print(f"minnal:      {total_minnal_io}")
    if total_claude_io > 0:
        io_pct_diff = ((total_minnal_io - total_claude_io) / total_claude_io) * 100
        print(f"Difference: {total_minnal_io - total_claude_io:+} ({io_pct_diff:+.1f}%)")

    # Check if within acceptable bounds
    # minnal using fewer tokens is always good (negative diff)
    # minnal using more tokens is acceptable up to MAX_OVERHEAD_PCT
    MAX_OVERHEAD_PCT = 50  # Allow up to 50% more tokens (for different system prompts)

    passed = True
    if pct_diff <= 0:
        print(f"\n✅ PASS: minnal uses {abs(pct_diff):.1f}% fewer tokens than Claude CLI")
    elif pct_diff <= MAX_OVERHEAD_PCT:
        print(f"\n✅ PASS: minnal uses {pct_diff:.1f}% more tokens (within {MAX_OVERHEAD_PCT}% threshold)")
    else:
        print(f"\n❌ FAIL: minnal uses {pct_diff:.1f}% more tokens (exceeds {MAX_OVERHEAD_PCT}% threshold)")
        passed = False

    # Per-prompt breakdown
    print("\nPer-prompt breakdown:")
    print(f"{'Prompt':<40} {'Claude':<10} {'minnal':<10} {'Diff':<10}")
    print("-" * 70)

    for r in results:
        prompt = r["prompt"][:37] + "..." if len(r["prompt"]) > 40 else r["prompt"]
        c_total = r["comparison"]["claude"]["total"]
        j_total = r["comparison"]["minnal"]["total"]
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare token usage between minnal and Claude Code CLI"
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
    ...
    print("Token Usage Comparison: minnal vs Claude Code CLI")
    print("=" * 50)
