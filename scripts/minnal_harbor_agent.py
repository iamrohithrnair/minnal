...
IN_CONTAINER_HOME = "/tmp/minnal-home"
IN_CONTAINER_RUNTIME = "/tmp/minnal-runtime"
IN_CONTAINER_INPUT = "/tmp/minnal-input"
IN_CONTAINER_OUTPUT = "/tmp/minnal-output"
IN_CONTAINER_BINARY = "/usr/local/bin/minnal"
IN_CONTAINER_LIB_DIR = f"{IN_CONTAINER_RUNTIME}/lib"
IN_CONTAINER_CA_BUNDLE = f"{IN_CONTAINER_HOME}/ca-certificates.crt"
DEFAULT_BINARY_PATH = "/tmp/minnal-compat-dist/minnal-linux-x86_64"
DEFAULT_OPENAI_AUTH_PATH = "~/.minnal/openai-auth.json"
CA_BUNDLE_CANDIDATES = (
    os.environ.get("MINNAL_HARBOR_CA_BUNDLE"),
...
OPENSSL_RUNTIME_LIBS = tuple(
    ...
    for lib in (
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("MINNAL_HARBOR_LIBSSL"),
                *_sibling_runtime_lib_candidates(MINNAL_BINARY, "libssl"),
                "/usr/lib/libssl.so.3",
        ...
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("MINNAL_HARBOR_LIBCRYPTO"),
                *_sibling_runtime_lib_candidates(MINNAL_BINARY, "libcrypto"),
...
def _benchmark_instruction_preamble() -> str:
    # Keep Harbor runs aligned with normal TUI/minnal-run prompting by default.
    # The legacy preamble can still be enabled explicitly for reproducing older
    # runs, but new benchmark runs should rely on minnal's normal system prompt
    # and the official Terminal-Bench task instruction.
    if os.environ.get("MINNAL_HARBOR_LEGACY_PREAMBLE"):
        return LEGACY_BENCHMARK_INSTRUCTION_PREAMBLE
    return os.environ.get("MINNAL_HARBOR_EXTRA_PREAMBLE", "")


def _load_task_hint() -> str:
    if not os.environ.get("MINNAL_HARBOR_ENABLE_HINTS"):
        return ""
    task_name = os.environ.get("MINNAL_HARBOR_CURRENT_TASK", "").strip()
    hints_path = os.environ.get("MINNAL_HARBOR_TASK_HINTS_FILE", "").strip()
    extra = os.environ.get("MINNAL_HARBOR_EXTRA_PREAMBLE", "").strip()
    parts: list[str] = []
    if extra:
        parts.append(extra)
...
class MinnalHarborAgent(BaseAgent):
    def __init__(self, logs_dir: Path, model_name: str | None = None, *args, **kwargs):
        ...
        if "/" in self._model_arg:
            self._provider_arg, self._minnal_model = self._model_arg.split("/", 1)
        else:
            self._provider_arg, self._minnal_model = "openai", self._model_arg

    @staticmethod
    def name() -> str:
        return "minnal-harbor"
    ...
    async def setup(self, environment: BaseEnvironment) -> None:
        ...
        await environment.upload_file(MINNAL_BINARY, IN_CONTAINER_BINARY)
        ...
        version_result = await environment.exec(
            ...
            env={
                ...
                "MINNAL_HOME": IN_CONTAINER_HOME,
                "MINNAL_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
    ...
    async def run(self, instruction: str, environment: BaseEnvironment, context: AgentContext) -> None:
        ...
        env = {
            ...
            "MINNAL_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
            "MINNAL_NO_TELEMETRY": "1",
            "MINNAL_PROVIDER": self._provider_arg,
            "MINNAL_MODEL": self._minnal_model,
            "MINNAL_OPENAI_REASONING_EFFORT": os.environ.get("MINNAL_OPENAI_REASONING_EFFORT", "high"),
            "MINNAL_OPENAI_SERVICE_TIER": os.environ.get("MINNAL_OPENAI_SERVICE_TIER", "priority"),
        ...
        result = await environment.exec(
            command=(
                ...
                '--provider "$MINNAL_PROVIDER" --model "$MINNAL_MODEL" '
        ...
        try:
            await environment.download_dir(IN_CONTAINER_OUTPUT, self.logs_dir / "minnal-output")
        ...
        metadata: dict[str, Any] = {
            ...
            "model": self._minnal_model,
            "minnal_binary": str(MINNAL_BINARY),
        }

        output_dir = self.logs_dir / "minnal-output"
        ...
        if payload is not None:
            ...
            metadata["minnal_result"] = payload
