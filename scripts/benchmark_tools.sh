...
# Run from the minnal repo root

set -euo pipefail

ITERATIONS=${1:-5}
RESULTS_FILE="/tmp/minnal_tool_benchmark_$(date +%Y%m%d_%H%M%S).csv"

echo "=== minnal Tool Call Benchmark ==="
...
# Find debug socket
DEBUG_SOCK="${MINNAL_DEBUG_SOCK:-/run/user/$(id -u)/minnal-debug.sock}"

if [ ! -S "$DEBUG_SOCK" ]; then
    echo "ERROR: Debug socket not found at $DEBUG_SOCK"
    echo "Make sure minnal is running with debug control enabled."
    exit 1
fi
