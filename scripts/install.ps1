...
.PARAMETER ArtifactExePath
    Use a local minnal.exe artifact instead of downloading from GitHub.
.PARAMETER ArtifactTgzPath
    Use a local minnal .tar.gz artifact instead of downloading from GitHub.
...
function Invoke-ProcessWithTimeout {
    ...
    $stderrPath = $null
    if ($CaptureOutput) {
        $stdoutPath = Join-Path $env:TEMP ("minnal-{0}-{1}-stdout.log" -f $FriendlyName, [guid]::NewGuid().ToString('N'))
        $stderrPath = Join-Path $env:TEMP ("minnal-{0}-{1}-stderr.log" -f $FriendlyName, [guid]::NewGuid().ToString('N'))
...
function Install-MinnalHotkey([string]$MinnalExePath) {
    ...
    $ps1Lines = @(
        '# minnal Alt+; global hotkey listener',
    ...
    $vbsPath = Join-Path $HotkeyDir "minnal-hotkey-launcher.vbs"
    $vbsContent = @(
    ...
    $startupShortcutPath = (Join-Path $startupDir "minnal-hotkey.lnk").Replace("'", "''")
    ...
    $shortcutLines = @(
        ...
        "`$shortcut.Description = 'minnal Alt+; hotkey listener'",
    ...
    Write-Info "Configured Alt+; to launch minnal in Alacritty"
    return $true
}
...
if ($Existing) {
    if ($Existing -match [regex]::Escape($VersionNum)) {
        Write-Info "minnal $Version is already installed - reinstalling"
    } else {
        Write-Info "Updating minnal $Existing -> $Version"
    }
} else {
    Write-Info "Installing minnal $Version"
}
...
$TempDir = Join-Path $env:TEMP "minnal-install-$(Get-Random)"
New-Item -ItemType Directory -Path $TempDir -Force | Out-Null

$DownloadMode = ""
$DownloadPath = Join-Path $TempDir "minnal.download"
...
if ($DownloadMode -eq "tar") {
    ...
    $SrcDir = Join-Path $TempDir "minnal-src"
...
Copy-Item -Path $DestBin -Destination (Join-Path $StableDir "minnal.exe") -Force
Set-Content -Path (Join-Path $BuildsDir "stable-version") -Value $VersionNum
Copy-Item -Path (Join-Path $StableDir "minnal.exe") -Destination $LauncherPath -Force
...
Write-Host ""
Write-Info "minnal $Version installed successfully!"
Write-Host ""
...
if ($configuredHotkey) {
    Write-Info "Global hotkey ready: Alt+; opens minnal in Alacritty"
    Write-Host ""
}

if (Get-Command minnal -ErrorAction SilentlyContinue) {
    Write-Info "Run 'minnal' to get started."
} else {
    Write-Host "  Open a new terminal window, then run:"
    Write-Host ""
    Write-Host "    minnal" -ForegroundColor Green
}
