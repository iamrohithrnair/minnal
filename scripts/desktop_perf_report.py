#!/usr/bin/env python3
"""Summarize Minnal Desktop persistent performance logs.

The desktop app writes JSONL events to ~/.cache/minnal/desktop/performance.log.
...
def default_log_path() -> Path:
    ...
    if xdg:
        return Path(xdg) / "minnal" / "desktop" / "performance.log"
    return Path.home() / ".cache" / "minnal" / "desktop" / "performance.log"
...
def summarize(events: list[dict[str, Any]], scope: str, malformed: int, top: int) -> dict[str, Any]:
    ...
    no_paint = top_events(events, "minnal-desktop-no-paint-profile", ("payload", "gap_ms"), top)
    frames = top_events(events, "minnal-desktop-frame-profile", ("payload", "worst_wall_ms"), top)
    session_events = top_events(events, "minnal-desktop-session-event-profile", ("payload", "ui_queue_delay_ms"), top)
    card_loads = top_events(events, "minnal-desktop-session-cards-load-profile", ("payload", "loaded_in_ms"), top)
    single_card_loads = top_events(events, "minnal-desktop-session-card-refresh-profile", ("payload", "loaded_in_ms"), top)
    preference_saves = top_events(events, "minnal-desktop-preferences-save-profile", ("payload", "saved_in_ms"), top)
    restores = top_events(events, "minnal-desktop-crashed-sessions-restore-profile", ("payload", "elapsed_ms"), top)
...
def print_text(summary: dict[str, Any], path: Path) -> None:
    print("Minnal Desktop performance report")
    print(f"log: {path}")
