...
Compares minnal's swarm (multi-agent coordination) with single-agent performance
...
Environment:
    Requires minnal server running with debug_control enabled:
        touch ~/.minnal/debug_control
        minnal serve
"""
...
def main():
    ...
    if not os.path.exists(DEBUG_SOCKET):
        ...
        print("Make sure minnal server is running with debug_control enabled:")
        print("  touch ~/.minnal/debug_control")
        print("  minnal serve")
        sys.exit(1)
