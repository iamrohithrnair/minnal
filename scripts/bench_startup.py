#!/usr/bin/env python3
"""Benchmark and optionally regression-check minnal startup time.

This script runs isolated startup measurements under a temporary MINNAL_HOME and
MINNAL_RUNTIME_DIR so it does not interfere with the user's real server, logs, or
credentials.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("binary", nargs="?", default="./target/release/minnal")
...
def measure_server_startup(binary: str, runs: int) -> list[float]:
    times: list[float] = []
    for _ in range(runs):
        root = tempfile.mkdtemp(prefix="minnal-server-bench-")
        env = isolated_env(root)
        socket_path = env["MINNAL_SOCKET"]
        proc = None
        try:
            start = time.perf_counter()
...
def measure_cold_client_startup(binary: str, runs: int) -> list[StartupProfile]:
    ...
    for _ in range(runs):
        root = tempfile.mkdtemp(prefix="minnal-cold-bench-")
        env = isolated_env(root)
        log_path = Path(env["MINNAL_HOME"]) / "logs" / f"minnal-{time.strftime('%Y-%m-%d')}.log"
        try:
            command = (
                f"{binary} --no-update --debug-socket "
                f"--socket {env['MINNAL_SOCKET']}"
            )
            subprocess.run(
