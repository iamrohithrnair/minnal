...
def minnal_debug_socket():
    """Find the active minnal debug socket path."""
    runtime_dir = os.environ.get("XDG_RUNTIME_DIR") or f"/run/user/{os.getuid()}"
    return os.path.join(runtime_dir, "minnal-debug.sock")
...
def dbg(command, session_id=None, timeout=TIMEOUT_SECS):
    ...
    return _send_recv(minnal_debug_socket(), req, timeout=timeout)
...
def test_reload_context_roundtrip():
    minnal_dir = pathlib.Path.home() / ".minnal"
    ctx_path = minnal_dir / "reload-context.json"
...
def test_reload_info_file():
    minnal_dir = pathlib.Path.home() / ".minnal"
    info_path = minnal_dir / "reload-info"
...
def test_canary_binary_path():
    ...
    manifest_path = home / ".minnal" / "build-manifest.json"
    ...
    if canary_hash:
        canary_binary = home / ".minnal" / "builds" / "canary" / "minnal"
...
def test_multiple_sessions_responsive():
    ...
    try:
        for i in range(N):
            s = create_session(f"/tmp/minnal-test-{i}")
            sessions.append(s)
...
def check_server_up():
    dbg_sock = minnal_debug_socket()
    if not os.path.exists(dbg_sock):
        raise RuntimeError(
            f"Debug socket not found at {dbg_sock}.\n"
            "  Start minnal server first:\n"
            "    minnal serve   (or just launch minnal in the repo)"
        )
    try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test minnal selfdev reload mechanism",
    ...
    print("\n╔══════════════════════════════════════════════════════════╗")
    print("║      minnal selfdev reload test suite                    ║")
    print("╚══════════════════════════════════════════════════════════╝\n")
